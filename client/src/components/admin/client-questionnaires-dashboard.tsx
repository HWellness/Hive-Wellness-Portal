...
export default function ClientQuestionnairesDashboard() {
  ...
  const [selectedQuestionnaire, setSelectedQuestionnaire] = useState<ClientQuestionnaire | null>(
    null
  );
  ...
  const filteredQuestionnaires = questionnaires.filter((questionnaire) => {
    const matchesSearch =
      ...
      questionnaire.responses?.personalInfo?.email
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      questionnaire.responses?.personalInfo?.firstName
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      questionnaire.responses?.personalInfo?.lastName
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
              ...
              try {
                ...
                if (result.success) {
                  ...
                  toast({
                    title: "Import Failed",
                    description:
                      result.error || "Could not import from HubSpot. Check API configuration.",
                    variant: "destructive",
                  });
                }
              } catch (error) {
                        ...
                        <div className="flex gap-2">
                          {questionnaire.assignedTherapistId && (
                              ...
                              onClick={() => {
                                ...
                                if (!clientEmail || !clientFirstName) {
                                  toast({
                                    ...
                                    description:
                                      "Client email or name is missing from questionnaire",
                                  });
                                  return;
                                }
                            ...
                            >
                              ...
                              {sendActivationEmailMutation.isPending
                                ? "Sending..."
