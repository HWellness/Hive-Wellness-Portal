...
export function AssignmentNotifications() {
  ...
  const [selectedNotification, setSelectedNotification] = useState<AssignmentNotification | null>(null);
  ...
  const urgentCount = notifications.filter((n: AssignmentNotification) => n.priority === 'urgent').length;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {notifications.filter((n: AssignmentNotification) => 
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {notification.type === 'assignment_needed' && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            console.log(`Redirecting to assignment for client: ${notification.clientName} (ID: ${notification.clientId})`);
                            // First mark as read, then redirect to assignment interface
                            handleNotificationAction(notification.id, 'mark_read');
                            setLocation(`/admin-services/client-assignment?clientId=${notification.clientId}&clientName=${encodeURIComponent(notification.clientName)}`);
                          }}
                          disabled={actionMutation.isPending}
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Button
              ...
              onClick={() => queryClient.invalidateQueries({ queryKey: ['/api/admin/assignment-notifications'] })}
            >
              <Bell className="h-5 w-5" />
            ...
            <Button
              ...
              onClick={() => {
                ...
                const event = new CustomEvent('navigate', { detail: { path: '/admin/client-therapist-assignment' } });
