...
export default function SMSStatusPanel() {
  const { data: twilioStatus, isLoading, error } = useQuery<{ config: TwilioConfig }>({
  ...
  const StatusIcon = ({ enabled }: { enabled: boolean }) => 
    enabled ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-gray-400" />;
  ...
  return (
    <Card data-testid="sms-status-panel">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Authorization Error */}
        {isUnauthorized && (
          <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg" data-testid="unauthorized-message">
        ...
        <div className="space-y-2" data-testid="sms-provider-section">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg" data-testid="twilio-status">
            <span className="text-sm">Twilio Integration</span>
            {twilioConfigured ? (
              <Badge className="bg-green-100 text-green-800 border-green-200" data-testid="twilio-configured">
                <CheckCircle2 className="h-3 w-3 mr-1" />
                Configured
              </Badge>
            ) : (
              <Badge variant="outline" className="border-gray-300" data-testid="twilio-not-configured">
        ...
        <div className="space-y-2" data-testid="channel-availability-section">
          ...
          {/* Therapist Channels */}
          <div className="space-y-2 p-3 bg-purple-50 border border-purple-100 rounded-lg" data-testid="therapist-channels">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="flex items-center gap-2" data-testid="therapist-whatsapp-status">
                ...
                <span className={therapistWhatsAppEnabled ? "" : "text-gray-500"}>WhatsApp</span>
                <Badge variant="outline" className="ml-auto text-xs">Off</Badge>
              </div>
            </div>
            <p className="text-xs text-gray-600 mt-2">
              Note: SMS/WhatsApp disabled for therapists per Step 46. Portal messages use email notifications instead.
            </p>
          </div>

          {/* Client Channels */}
          <div className="space-y-2 p-3 bg-blue-50 border border-blue-100 rounded-lg" data-testid="client-channels">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="flex items-center gap-2" data-testid="client-sms-status">
                ...
                <span>SMS</span>
                <Badge className="ml-auto bg-green-100 text-green-800 border-green-200 text-xs">Active</Badge>
              </div>
        ...
        <div className="text-xs text-gray-500 pt-2 border-t" data-testid="implementation-note">
          ...
          Therapist SMS/WhatsApp channels disabled; portal messages trigger email notifications instead.
        </div>
          </>
        )}
      </CardContent>
