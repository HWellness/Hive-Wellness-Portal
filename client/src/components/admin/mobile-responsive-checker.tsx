...
const MobileResponsiveChecker: React.FC<MobileResponsiveCheckerProps> = ({ user }) => {
  ...
  const testViewports = [
    ...
    { id: 'iphone-12', name: 'iPhone 12/13/14', width: 390, height: 844, device: 'mobile' as const },
    { id: 'iphone-12-pro-max', name: 'iPhone 12 Pro Max', width: 428, height: 926, device: 'mobile' as const },
    { id: 'samsung-galaxy-s21', name: 'Samsung Galaxy S21', width: 360, height: 800, device: 'mobile' as const },
    { id: 'google-pixel-5', name: 'Google Pixel 5', width: 393, height: 851, device: 'mobile' as const },
    ...
    { id: 'android-tablet', name: 'Android Tablet', width: 800, height: 1280, device: 'tablet' as const },
    
    // Desktop
    { id: 'desktop-1080p', name: '1080p Desktop', width: 1920, height: 1080, device: 'desktop' as const },
  ...
  const generateRecommendations = () => {
    ...
    if (allIssues.some(issue => issue.includes('buttons may be too small'))) {
      recommendations.push('Increase minimum touch target size to 44px for better mobile accessibility');
    }
    
    if (allIssues.some(issue => issue.includes('horizontal scrolling'))) {
      recommendations.push('Implement responsive breakpoints and flexible layouts to prevent horizontal overflow');
    }
    
    if (allIssues.some(issue => issue.includes('load time'))) {
      recommendations.push('Optimize images and implement lazy loading for better mobile performance');
    }
    
    if (allIssues.some(issue => issue.includes('tablet space'))) {
      recommendations.push('Create tablet-specific layouts to better utilize available screen space');
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-gray-900">Mobile Responsiveness Checker</h1>
          <p className="text-gray-600 mt-1">Test Hive Wellness portal across multiple devices and screen sizes</p>
        </div>
            ...
            <div className="space-y-4">
              ...
              {currentTest && (
                <div className="flex items-center gap-3 text-sm text-gray-600">
                  ...
                  <span>Testing {currentTest.name} ({currentTest.viewport.width}×{currentTest.viewport.height})</span>
                </div>
              )}
            </div>
          </CardContent>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center gap-3 capitalize">
                    ...
                    <Badge variant="outline">
                      {deviceTests.filter(t => t.status === 'pass').length}/{deviceTests.length} Passed
                    </Badge>
                  </CardTitle>
                </CardHeader>
                      ...
                      <div key={test.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{test.viewport.width}×{test.viewport.height}px</p>
                          </div>
                        </div>
                        ...
                        <div className="text-right">
                          ...
                          {test.issues.length > 0 && (
                            <p className="text-sm text-red-600">{test.issues.length} issue{test.issues.length > 1 ? 's' : ''}</p>
                          )}
                        </div>
                      </div>
                    ))}
            ...
            <div className="space-y-3">
              ...
              {generateRecommendations().length === 0 && (
                <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-sm text-green-900">Great work! No major responsive issues detected.</p>
                </div>
              )}
            </div>
          </CardContent>
