...
import { 
  Mail, Eye, Edit, Save, Plus, Trash2, Send, AlertCircle, CheckCircle, X, 
  FileText, Copy, TestTube, Upload, Paperclip, Type, FileImage, RefreshCw
} from 'lucide-react';
...
export default function EmailTemplateManagementSimple({ onBackToDashboard }: EmailTemplateManagementProps) {
  ...
  const { data: templates = [], isLoading, error, refetch } = useQuery<EmailTemplate[]>({
  ...
  const testTemplateMutation = useMutation({
    mutationFn: async ({ templateId, testEmail }: { templateId: string; testEmail: string }) => {
      const response = await apiRequest('POST', `/api/admin/email-templates/${templateId}/test`, { testEmail });
      if (!response.ok) {
        const errorData = await response.json();
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Create and manage automated email templates for your practice</p>
        </div>
            ...
            {templates.length === 0 ? (
              ...
              templates.map((template) => (
                ...
                >
                  ...
                  <Badge className={`${getTypeColor(template.type)} mb-2 text-xs`} variant="secondary">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>
                {showCreateForm ? 'Create New Template' : 
                 selectedTemplate ? (isEditing ? 'Edit Template' : 'Template Details') : 
                 'Select a Template'}
              </CardTitle>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    You can use variables like {'{{firstName}}'}, {'{{lastName}}'}, {'{{therapistName}}'} in your message
                  </p>
                </div>
                ...
                <div className="flex gap-2 pt-4 border-t">
                  <Button 
                    ...
                    disabled={createMutation.isPending || !formData.name.trim() || !formData.subject.trim() || !formData.content.trim()}
                    className="bg-purple-600 hover:bg-purple-700"
                  >
                    <Save className="w-4 h-4 mr-2" />
              ...
              <div className="space-y-4">
                  ...
                  <Alert className="border-purple-200 bg-purple-50">
                    ...
                    <AlertDescription>
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <Button
                          size="sm"
                          onClick={() => testTemplateMutation.mutate({ templateId: selectedTemplate.id, testEmail })}
                ...
                {isEditing ? (
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={(e) => setSelectedTemplate({...selectedTemplate, signature: e.target.value})}
                        rows={4}
                        className="mt-1"
                        placeholder="Best regards,&#10;Dr. {{doctorName}}&#10;Hive Wellness"
                      />
                  ...
                  <>
                    ...
                    {selectedTemplate.signature && (
                      <div>
                        ...
                        <div className="mt-2 p-4 border rounded-lg bg-white">
                          <p className="whitespace-pre-wrap text-sm">{selectedTemplate.signature}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedTemplate.attachments && selectedTemplate.attachments.length > 0 && (
                      <div>
                        <Label className="text-sm font-medium text-gray-500">Attachments ({selectedTemplate.attachments.length})</Label>
                        <div className="mt-2 space-y-2">
                    ...
                    {selectedTemplate.usage !== undefined && (
                      <div className="text-sm text-gray-500">
                        ...
                        {selectedTemplate.lastUsed && (
                          <span>, last used on {new Date(selectedTemplate.lastUsed).toLocaleDateString()}</span>
                        )}
                      </div>
                    )}
                  </>
