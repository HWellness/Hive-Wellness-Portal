...
export default function TherapistStatusManager() {
  ...
  const [passwordModal, setPasswordModal] = useState<{ isOpen: boolean; email: string; password: string; isNewAccount: boolean }>({ 
    isOpen: false, 
    email: '', 
    password: '', 
    isNewAccount: false 
  ...
  const { data: enquiries = [], isLoading, refetch } = useQuery<TherapistEnquiry[]>({
  ...
  const updateStatusMutation = useMutation({
    ...
    onMutate: async ({ id, status }) => {
      ...
      queryClient.setQueryData(['/api/admin/therapist-enquiries'], (old: TherapistEnquiry[] | undefined) => {
        if (!old) return old;
        return old.map(enquiry => 
  ...
  const updateTierMutation = useMutation({
    ...
    onError: (error: any, variables, context) => {
      ...
      const errorMessage = error?.message?.includes('429') || error?.status === 429
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'enquiry_received': return <Clock className="w-4 h-4" />;
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Manage therapist application statuses and account creation</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { status: 'enquiry_received', label: 'Enquiry Received', count: enquiries.filter(e => e.status === 'enquiry_received').length },
          { status: 'under_review', label: 'Under Review', count: enquiries.filter(e => e.status === 'under_review').length },
          { status: 'approved', label: 'Approved', count: enquiries.filter(e => e.status === 'approved').length },
          { status: 'rejected', label: 'Rejected', count: enquiries.filter(e => e.status === 'rejected').length }
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="space-y-2 my-3">
                          {enquiry.personality_description && (
                            <div className="bg-blue-50 p-3 rounded-md">
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  <span className="text-sm font-medium text-blue-800">Personality: </span>
                                  <span className="text-sm text-blue-700">{enquiry.personality_description}</span>
                                </div>
                              </div>
                            </div>
                          )}
                          ...
                          {enquiry.therapy_specialisations && enquiry.therapy_specialisations.length > 0 && (
                            <div className="bg-purple-50 p-3 rounded-md">
                              <div className="flex items-start gap-2">
                                <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                                <div>
                                  <span className="text-sm font-medium text-purple-800">Specialisations: </span>
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 lg:min-w-0">
                        ...
                        >
                          <SelectTrigger className={`w-full sm:w-48 ${updateStatusMutation.isPending ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                        ...
                        <div className="w-full sm:w-52">
                          ...
                          >
                            <SelectTrigger className={`w-full ${(updateTierMutation.isPending || isAnyTierUpdating) ? 'opacity-50 cursor-not-allowed' : ''}`}>
                              <SelectValue placeholder="Select tier..." />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="counsellor">Counsellor (£{TIER_PRICING.counsellor})</SelectItem>
                              <SelectItem value="psychotherapist">Psychotherapist (£{TIER_PRICING.psychotherapist})</SelectItem>
                              <SelectItem value="psychologist">Psychologist (£{TIER_PRICING.psychologist})</SelectItem>
                              <SelectItem value="specialist">Specialist (£{TIER_PRICING.specialist})</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        ...
                        <div className="flex gap-2">
                          {enquiry.status === 'approved' && !enquiry.account_created && (
                            <Button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                console.log(`Creating account for single therapist: ${enquiry.email} (ID: ${enquiry.id})`);
                          ...
                          {enquiry.account_created && (
                            <Button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                console.log(`Reset button clicked for ${enquiry.email} (ID: ${enquiry.id})`);
      ...
      <Dialog open={passwordModal.isOpen} onOpenChange={(open) => setPasswordModal(prev => ({ ...prev, isOpen: open }))}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-md text-sm text-blue-800">
              <strong>Note:</strong> The therapist should log in at the main portal and change this password immediately for security.
            </div>
          </div>
          
          <DialogFooter>
            <Button 
              onClick={() => setPasswordModal({ isOpen: false, email: '', password: '', isNewAccount: false })}
              className="w-full"
            >
              Close
            </Button>
