...
import { formatCurrency, formatCurrencyCompact, type SupportedCurrency, type CurrencyAmount } from "@/lib/currency";
...
export function CostTrendsChart({ data, timeRange, currency = 'GBP' }: CostTrendsChartProps) {
  const [selectedMetric, setSelectedMetric] = useState<'totalCost' | 'costPerAppointment' | 'therapistCount'>('totalCost');
        ...
        <div>
          ...
          <div className="flex items-center mt-2">
            ...
            <span className={`text-sm ${trendColor}`}>
              {percentage.toFixed(1)}% {trend === 'increasing' ? 'increase' : trend === 'decreasing' ? 'decrease' : 'stable'}
            </span>
          </div>
        </div>
              ...
              content={
                <ChartTooltipContent
                  formatter={(value, name) => {
                    const numericValue = typeof value === 'object' && value?.amount !== undefined ? value.amount : Number(value);
                    return [
                      selectedMetric === 'therapistCount' 
                        ? numericValue 
...
export function TherapistCostDistribution({ data, currency = 'GBP' }: TherapistCostDistributionProps) {
  const [viewType, setViewType] = useState<'individual' | 'planType'>('individual');

  const processedData = viewType === 'individual' 
    ? data.slice(0, 8).map((item, index) => {
        const costAmount = typeof item.totalMonthlyCost === 'object' ? item.totalMonthlyCost.amount : item.totalMonthlyCost;
        return {
                    ...
                    return (
                      <div className="bg-white p-3 border rounded-lg shadow-lg">
                        ...
                        <p className="text-sm text-gray-600">
                          {formatCurrency(data.value, currency)} ({((data.value / totalCost) * 100).toFixed(1)}%)
                        </p>
                        {data.planType && (
...
export function UsageEfficiencyChart({ data, currency = 'GBP' }: UsageEfficiencyChartProps) {
  ...
  const processedData = data.map(item => {
    const costPerAppointment = typeof item.costPerAppointment === 'object' ? item.costPerAppointment.amount : item.costPerAppointment;
    const totalCost = typeof item.totalMonthlyCost === 'object' ? item.totalMonthlyCost.amount : item.totalMonthlyCost;
        ...
        <ChartContainer config={efficiencyConfig}>
          <ScatterChart data={processedData}>
            ...
            <YAxis 
              ...
              label={{ value: `Cost per Appointment (${currency})`, angle: -90, position: 'insideLeft' }}
            />
            <ChartTooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
          ...
          <div className="p-3 bg-amber-50 rounded-lg">
            ...
            <p className="text-amber-700 text-xs">
              Low utilization or high cost per appointment
            </p>
          </div>
        </div>
      </CardContent>
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <Badge variant={data.utilizationPercentage >= 90 ? 'destructive' : 'default'} className={status.color}>
              {status.label}
            </Badge>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <p className={`text-2xl font-bold ${data.variance >= 0 ? 'text-red-600' : 'text-green-600'}`}>
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            {alertThresholds.map((threshold, index) => (
              <div key={index} className="flex items-center justify-center p-2 bg-white rounded">
                <span className={data.utilizationPercentage >= threshold ? 'text-red-600 font-medium' : 'text-gray-600'}>
                  {threshold}%
                </span>
              </div>
            ))}
      ...
      <CardContent>
        <ChartContainer config={comparisonConfig}>
          <BarChart data={data.slice().reverse()}>
            ...
            <Bar dataKey="budgetAmount" fill={comparisonConfig.budgetAmount.color} radius={[2, 2, 0, 0]} />
            <Bar dataKey="actualCost" fill={comparisonConfig.actualCost.color} radius={[2, 2, 0, 0]} />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
        ...
        <div className="space-y-3">
          {sortedData.map((therapist, index) => (
            <div key={index} className="flex items-center space-x-4">
              <div className="w-32 text-sm font-medium truncate" data-testid={`text-therapist-${index}`}>
              ...
              <div className="flex-1 flex items-center space-x-2">
                ...
                <div className="w-20 text-right text-sm font-medium" data-testid={`text-cost-${index}`}>
