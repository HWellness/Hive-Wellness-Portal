...
import { Send, User, Search, MessageCircle, Archive, MoreHorizontal, Trash2, Bell, BellOff, UserCheck, Shield, AlertTriangle, Check, X } from "lucide-react";
...
export default function MessagingService({ user }: MessagingProps) {
  ...
  const createConversationMutation = useMutation({
    mutationFn: async ({ recipientEmail, initialMessage }: { recipientEmail?: string; initialMessage?: string }) => {
  ...
  const sendMessageMutation = useMutation({
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.previousMessages && selectedConversation) {
        queryClient.setQueryData([`/api/messages/${selectedConversation}`], context.previousMessages);
      }
  ...
  const handleConnectionAction = async (requestId: string, action: 'accept' | 'decline', clientName: string) => {
    try {
  ...
  const selectedConversationData = Array.isArray(conversations) ? conversations.find((conv: Conversation) => conv.id === selectedConversation) : null;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-amber-800 font-medium">
              Please do not include personal information in messages. All communications are monitored by Hive Wellness staff.
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <Button 
              ...
              <div className="p-4">
                {user.role === 'therapist' ? (
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-gradient-to-br from-hive-purple to-purple-600 rounded-full flex items-center justify-center">
                              <span className="text-white font-semibold text-sm">
                                {(client.name || client.first_name || client.email || 'U').charAt(0).toUpperCase()}
                              </span>
                            </div>
                            <div>
                              <h4 className="font-semibold text-gray-900">
                                {client.name || `${client.first_name || ''} ${client.last_name || ''}`.trim() || client.email}
                              </h4>
                  ...
                  <div className="text-center">
                    <p className="text-gray-600">You can message your therapist through existing conversations or wait for them to start a new one.</p>
                    <Button 
                      onClick={() => setShowNewMessageDialog(false)} 
                      className="mt-4"
                    >
          ...
          <div className="flex-1 overflow-y-auto">
            {conversationsLoading ? (
              ...
              <div className="text-center py-12 px-6">
                ...
                <h3 className="font-display font-semibold text-gray-900 mb-2">No conversations yet</h3>
                <p className="text-gray-600 text-sm">Your therapist will message you here, or you can start a conversation.</p>
              </div>
            ) : (
              <div className="p-2">
              ...
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900">{selectedConversationData.participantName}</h3>
                      <p className="text-sm text-gray-600 capitalize">{selectedConversationData.participantRole}</p>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 min-h-0">
                {messagesLoading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <h3 className="font-display font-semibold text-gray-900 mb-2">Start the conversation</h3>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <p className="text-sm text-green-700 mt-2">
                All messages are stored in plaintext and are accessible by Hive Wellness administrative staff for quality assurance and monitoring purposes.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Messages are encrypted in transit using TLS 1.3</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">All conversations are monitored by Hive Wellness staff for quality assurance and safety</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Authorised Hive Wellness personnel can access message content for therapeutic oversight and platform safety</p>
                </div>
              </div>
            </div>
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            <div className="space-y-4 py-4 max-h-96 overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                {[
                  { id: '1', clientName: 'Emma Johnson', message: 'Thank you for yesterday\'s session, it really helped...', time: '5 minutes ago', unread: true },
                  { id: '2', clientName: 'Michael Chen', message: 'I wanted to follow up on the breathing exercises...', time: '2 hours ago', unread: true },
                  { id: '3', clientName: 'Sarah Williams', message: 'Could we reschedule our next appointment?', time: '1 day ago', unread: false }
                ].map((notification) => (
                  <div key={notification.id} className="flex items-start space-x-3 p-3 bg-hive-purple/10 border border-hive-purple/30 rounded-lg">
                    <div className="w-10 h-10 bg-gradient-to-br from-hive-purple to-purple-600 rounded-full flex items-center justify-center">
                      <span className="text-white font-semibold text-sm">
                    ...
                    <div className="flex-1">
                      ...
                      {notification.unread && (
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-xs bg-hive-purple/20 text-hive-purple px-2 py-1 rounded-full">New</span>
                          <Button size="sm" variant="outline" className="h-7">Reply</Button>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="space-y-3">
                ...
                {[
                  { id: '1', clientName: 'David Thompson', message: 'Referred by Dr. Smith for anxiety management', time: '30 minutes ago', status: 'pending' },
                  { id: '2', clientName: 'Lisa Anderson', message: 'Looking for help with work-related stress', time: '3 hours ago', status: 'pending' }
                ].map((request) => (
                  <div key={request.id} className="flex items-start space-x-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-2">
                        <Button 
                          ...
                          onClick={() => handleConnectionAction(request.id, 'accept', request.clientName)}
                        >
                        ...
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => handleConnectionAction(request.id, 'decline', request.clientName)}
                        >
