...
import { 
  Calendar as CalendarIcon, Clock, UserIcon, Plus, Video, 
  CheckCircle, AlertCircle, ChevronLeft, ChevronRight, CreditCard,
  Users, Bell, History, Info
} from "lucide-react";
...
const stripePublicKey = import.meta.env.VITE_STRIPE_TEST_PUBLIC_KEY || import.meta.env.VITE_STRIPE_PUBLIC_KEY;
...
export default function AppointmentBookingComplete({ user }: AppointmentBookingProps) {
  ...
  const [sessionType, setSessionType] = useState<'consultation' | 'therapy' | 'psychological' | 'specialist'>('therapy');
  ...
  const { data: config = { features: { allowPastBooking: false, pastBookingWindowDays: 14 } } } = useQuery({
    queryKey: ['/api/config'],
    retry: false,
  });
  ...
  const getSessionPrice = (sessionType: 'consultation' | 'therapy' | 'psychological' | 'specialist') => {
    switch (sessionType) {
      case 'consultation':
  ...
  const getNumericPrice = (sessionType: 'consultation' | 'therapy' | 'psychological' | 'specialist') => {
    switch (sessionType) {
      case 'consultation':
        return 65.00;
      case 'therapy':
  ...
  const getSessionDuration = (sessionType: 'consultation' | 'therapy' | 'psychological' | 'specialist') => {
    switch (sessionType) {
      case 'consultation':
  ...
  const createBulkAppointmentsMutation = useMutation({
    mutationFn: async (bulkData: any) => {
      const { bulkBookingType, numberOfSessions, recurringTime, startDate, selectedTherapist } = bulkData;
  ...
  const handlePaymentSuccess = async () => {
    if (pendingAppointmentData) {
      ...
      const pollForBulkAppointments = async (maxAttempts = 15, initialInterval = 1000) => {
        ...
        const poll = async () => {
          attempts++;
          console.log(`🔄 Polling for ${expectedCount} bulk appointments (attempt ${attempts}/${maxAttempts})`);
          ...
          try {
            ...
            if (newAppointments >= expectedCount) {
              console.log(`✅ All ${expectedCount} bulk appointments created after ${Date.now() - startTime}ms`);
            ...
            if (attempts < maxAttempts) {
              console.log(`⏳ Found ${newAppointments}/${expectedCount} appointments, continuing...`);
              interval = Math.min(interval * 1.2, 3000); // Exponential backoff, max 3s
              setTimeout(poll, interval);
            } else {
              console.error(`❌ Max polling attempts reached. Found ${newAppointments}/${expectedCount} appointments`);
              toast({
  ...
  const therapists = user.role === 'client' 
    ? (Array.isArray(assignedConnections) ? assignedConnections.map((conn: any) => ({
        ...conn.therapistProfile,
        userId: conn.therapistId
      })) : [])
    : (Array.isArray(assignedConnections) ? assignedConnections : []); // For therapists, show assigned clients
  ...
  useEffect(() => {
    ...
    if (user.role === 'client' && !selectedTherapist && Array.isArray(therapists) && !connectionsLoading) {
  ...
  const joinVideoSessionMutation = useMutation({
    ...
    onSuccess: (data: any, appointmentId) => {
      if (data.success && data.meetingUrl) {
        ...
        const meetWindow = window.open(data.meetingUrl, '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes');
        if (!meetWindow) {
          toast({
  ...
  const { data: availableTimeSlotsResponse, isLoading: timeSlotsLoading, refetch: refetchTimeSlots } = useQuery({
  ...
  const handleBookAppointment = () => {
    ...
    if (user.role === 'client' && Array.isArray(therapists) && therapists.length > 0) {
      const isAssignedTherapist = therapists.some((t: any) => t.userId === selectedTherapist.userId);
      if (!isAssignedTherapist) {
        toast({
          title: "Booking Restricted",
          description: "You can only book sessions with your assigned therapist. Please contact support if you need assistance.",
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <div className="container mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">Schedule and manage your therapy sessions with integrated video calling</p>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-hive-purple text-white rounded-full flex items-center justify-center font-bold text-sm">1</div>
              <p className="text-sm">Click "Book Appointment" to start the booking process</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-hive-purple text-white rounded-full flex items-center justify-center font-bold text-sm">2</div>
              <p className="text-sm">Select your preferred date from the calendar</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-hive-purple text-white rounded-full flex items-center justify-center font-bold text-sm">3</div>
              <p className="text-sm">Choose an available time slot</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-hive-purple text-white rounded-full flex items-center justify-center font-bold text-sm">4</div>
              <p className="text-sm">Select your preferred therapist</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-hive-purple text-white rounded-full flex items-center justify-center font-bold text-sm">5</div>
              <p className="text-sm">Complete payment to confirm your booking</p>
            </div>
          </div>
          <div className="mt-4 p-3 bg-green-50 rounded-lg">
            <p className="text-sm text-green-800">✅ Only available time slots are displayed - no more booking conflicts!</p>
          </div>
        </CardContent>
      </Card>
      ...
      {(user.role === 'client' || user.role === 'therapist' || user.role === 'institution') && (
        <div className="flex justify-center gap-4">
          <Button onClick={() => setShowBookingDialog(true)} className="bg-hive-purple hover:bg-hive-purple/90 px-8 py-3 text-lg">
            <Plus className="mr-2 h-5 w-5" />
            {user.role === 'therapist' ? 'Schedule Session' : 
             user.role === 'institution' ? 'Book Video Session' : 'Book Appointment'}
          </Button>
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              {(user.role === 'client' || user.role === 'therapist' || user.role === 'institution') && (
                <Button className="mt-4" onClick={() => setShowBookingDialog(true)}>
                  {user.role === 'therapist' ? 'Schedule First Session' : 
                   user.role === 'institution' ? 'Book First Video Session' : 'Book Your First Session'}
                </Button>
              )}
            </div>
          ) : (
      ...
      <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
        <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl font-primary text-hive-purple">Book New Appointment</DialogTitle>
          </DialogHeader>
                ...
                <Card className="p-4 border-2 border-dashed border-amber-200 bg-amber-50/50">
                  ...
                  {isPastBookingMode && (
                    <div className="space-y-3">
                      <div className="flex items-start gap-2 p-3 bg-amber-100/50 rounded-lg">
                        ...
                        <div className="text-sm text-amber-800">
                          ...
                          <ul className="text-xs space-y-1">
                            <li>• You can record sessions up to {(config as any)?.features?.pastBookingWindowDays || 14} days in the past</li>
                            <li>• Payment is required immediately as this is for a completed session</li>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="past-booking-reason" className="text-sm font-medium text-amber-800">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                  {isPastBookingMode ? (
                    ...
                    ['9:00 AM', '10:00 AM', '11:00 AM', '12:00 PM', '1:00 PM', '2:00 PM', '3:00 PM', '4:00 PM'].map((slot: string) => (
                      <Button
                        key={slot}
              ...
              <div>
                ...
                {connectionsLoading ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-lg font-semibold text-primary">{getSessionPrice(sessionType)}</span>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {Array.isArray(therapists) && therapists.map((therapist: any) => (
                      <Card
                        key={therapist.id}
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button 
                  ...
                  disabled={!selectedTime || !selectedTherapist || createPaymentIntentMutation.isPending}
                  className="flex-1"
              ...
              <PaymentForm
                ...
                onSuccess={() => {
                  ...
                  toast({
                    ...
                    description: "Your appointment is being created. You'll receive confirmation shortly!",
                    duration: 5000,
                  });
                  ...
                  const pollForNewAppointment = async (maxAttempts = 20, initialInterval = 800) => {
                    ...
                    const poll = async () => {
                      attempts++;
                      console.log(`🔄 Polling for new appointment (attempt ${attempts}/${maxAttempts})`);
                      
                      try {
                        await refetchAppointments();
                        
                        // Get fresh appointment data using the correct query key
                        const freshAppointments = await queryClient.getQueryData(['/api/appointments']) || [];
                        const hasNewAppointment = Array.isArray(freshAppointments) && freshAppointments.length > initialAppointmentCount;
                        
                        if (hasNewAppointment) {
                          console.log(`✅ New appointment created after ${Date.now() - startTime}ms`);
                          await queryClient.invalidateQueries({ queryKey: ['/api/video-sessions'] });
                          await queryClient.invalidateQueries({ queryKey: ['/api/appointments'] });
                          await queryClient.invalidateQueries({ queryKey: ['/api/client/dashboard'] });
                        ...
                        if (attempts < maxAttempts) {
                          ...
                          toast({
                            ...
                            description: "Your appointment may have been created. Please refresh the page.",
                            variant: "destructive",
                          });
                        }
                      } catch (error) {
...
const PaymentForm = ({ pendingAppointmentData, onSuccess, onCancel }: {
  ...
  const createAppointmentFromPaymentMutation = useMutation({
    mutationFn: async ({ paymentIntentId, appointmentData }: { paymentIntentId: string, appointmentData: any }) => {
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description: error.message || "Could not create appointment after payment. Please contact support.",
        variant: "destructive",
      });
    }
  });
  ...
  const calculatePaymentAmount = () => {
    if (pendingAppointmentData?.bulkBookingType && pendingAppointmentData?.numberOfSessions) {
      ...
      const sessionPrice = (pendingAppointmentData?.sessionType === 'consultation' || pendingAppointmentData?.sessionType === 'introduction') ? 65.00 : 80.00;
      ...
      return (pendingAppointmentData?.sessionType === 'consultation' || pendingAppointmentData?.sessionType === 'introduction') ? '£65.00' : '£80.00';
    }
  };
          ...
          {pendingAppointmentData?.bulkBookingType ? (
            <>
              <div className="flex justify-between">
                ...
                <span className="capitalize">{pendingAppointmentData.numberOfSessions} sessions ({pendingAppointmentData.bulkBookingType})</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span>{pendingAppointmentData?.startDate ? new Date(pendingAppointmentData.startDate).toLocaleDateString('en-GB') : ''}</span>
              </div>
              <div className="flex justify-between">
                <span>Time:</span>
            ...
            <>
              ...
              <div className="flex justify-between">
                <span>Date:</span>
                <span>{pendingAppointmentData?.scheduledAt ? new Date(pendingAppointmentData.scheduledAt).toLocaleDateString('en-GB') : ''}</span>
              </div>
