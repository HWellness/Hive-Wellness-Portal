...
export default function TherapistDashboard({ user, onNavigateToService }: TherapistDashboardProps) {
  ...
  const transformDashboardData = (apiData: any) => {
    ...
    const uniqueAppointments = apiData.upcomingAppointments
      ? apiData.upcomingAppointments.filter((apt: any, index: number, array: any[]) => {
          // Remove duplicates by ID and ensure valid appointment data
          return (
            apt.id && apt.scheduledAt && array.findIndex((a: any) => a.id === apt.id) === index
          );
        })
      : [];
    ...
    return {
      ...
      upcomingSessions: uniqueAppointments.map((apt: any) => ({
        ...
        time: new Date(apt.scheduledAt).toLocaleTimeString("en-GB", {
          hour: "2-digit",
          minute: "2-digit",
        }),
  ...
  const displayedSessions =
    viewMode === "overview" ? allSessions.slice(0, MAX_OVERVIEW_SESSIONS) : allSessions;
                          ...
                          {profileStatus?.profileDeactivated && (
                            <div className="p-4 bg-red-50 rounded-lg">
                              ...
                              <p className="text-sm text-red-600">
                                Deactivated:{" "}
                                {profileStatus.deactivatedAt
                                  ? new Date(profileStatus.deactivatedAt).toLocaleDateString()
                                  : "Unknown"}
                              </p>
                            </div>
                          )}
                          ...
                          {!profileStatus?.profileDeactivated && (
                            <div className="space-y-3">
                              ...
                              <Button
                                ...
                                disabled={
                                  !deactivationReason.trim() || profileStatusMutation.isPending
                                }
                                variant="destructive"
                                className="w-full"
                              >
                                {profileStatusMutation.isPending
                                  ? "Deactivating..."
                                  : "Deactivate Profile"}
                              </Button>
                            </div>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="text-center">
                    <div className="text-3xl font-bold">
                      £{data.overview.totalEarnings.toLocaleString()}
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs font-secondary text-hive-black/50">
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-primary font-bold text-hive-black">
                        {data.overview.sessionsThisWeek}
                      </p>
                      <p className="text-xs font-secondary text-hive-black/50">
                        sessions completed
                      </p>
            ...
            <Card className="bg-gradient-to-r from-green-50 to-emerald-50 border-green-200 shadow-lg">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="text-center">
                    ...
                    {payoutInfo?.pendingPayouts > 0 && (
                      <div className="text-xs text-green-500 mt-1">
                        {payoutInfo.pendingPayouts} payment
                        {payoutInfo.pendingPayouts > 1 ? "s" : ""} ready
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-center">
                    <Dialog
                      open={isInstantPayoutDialogOpen}
                      onOpenChange={setIsInstantPayoutDialogOpen}
                    >
                      <DialogTrigger asChild>
                        <Button
                          ...
                          disabled={
                            ...
                            parseFloat(payoutInfo?.availableBalance?.replace("£", "") || "0") <= 0
                          }
                            ...
                            <>
                              ...
                              <div className="text-center space-y-2">
                                <Button
                                  onClick={() => {
                                    const amount = parseFloat(
                                      payoutInfo?.availableBalance?.replace("£", "") || "0"
                                    );
                                    if (amount > 0) {
                                ...
                                <p className="text-xs text-gray-500">
                                  By confirming, you agree to the 1% processing fee for instant
                                  access.
                                </p>
                              </div>
                            </>
                ...
                {payoutHistory &&
                  ...
                  payoutHistory.payouts.filter((p: any) => p.payoutType === "instant").length >
                    0 && (
                    <div className="mt-6">
                      ...
                      displayedSessions
                        ...
                        .map((session: any) => {
                          ...
                          if (isNaN(sessionTime.getTime())) {
                            console.warn("Invalid session time constructed:", {
                              session,
                              sessionTime,
                            });
                            return null;
                          ...
                          return (
                            <div
                              key={session.id}
                              className="flex items-center justify-between p-4 bg-hive-light-blue/10 rounded-lg border border-hive-purple/10"
                            >
                                ...
                                <div className="flex items-center space-x-3">
                                  <div className="flex-1">
                                    <div className="flex items-center space-x-2 mb-1">
                                      <h4
                                        className="font-medium cursor-pointer hover:text-hive-purple transition-colors text-hive-black"
                                    ...
                                    <p className="text-xs text-hive-purple mt-1">
                                      Click name for client management
                                    </p>
                                  </div>
                                </div>
                              </div>
                                ...
                                <div className="flex items-center space-x-2">
                                  {canJoin ? (
                                    ...
                                    >
                                      ...
                                      {joinVideoSessionMutation.isPending
                                        ? "Joining..."
                                        : "Join Session"}
                                    </Button>
                                  ) : (
                      ...
                      {data.clientAlerts.map((alert: any) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p
                                className="font-medium text-sm text-hive-black cursor-pointer hover:text-hive-purple transition-colors"
                                onClick={() => onNavigateToService?.("client-matching")}
                              >
                                {alert.client}
                              </p>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium text-hive-black">
                ...
                {clientsLoading ? (
                  ...
                  <div className="space-y-4">
                    {assignedClients.map((client: any) => (
                      <div
                        key={client.id}
                        className="flex items-center justify-between p-4 bg-hive-light-blue/10 rounded-lg border border-hive-light-blue/20"
                      >
                        <div className="flex-1">
                          <div className="flex items-center space-x-3">
                            <div>
                              <h4
                                className="font-medium text-hive-black cursor-pointer hover:text-hive-purple transition-colors"
                                onClick={() => onNavigateToService?.("client-matching")}
                              >
                          ...
                          {client.profileData && (
                            <div className="mt-2 text-sm text-hive-black/60">
                              {client.profileData.preferredGender && (
                                <span className="mr-4">
                                  Preferred Gender: {client.profileData.preferredGender}
                                </span>
                              )}
                              {client.profileData.religion && (
                                <span className="mr-4">
                                  Religion: {client.profileData.religion}
                                </span>
                              )}
                              {client.profileData.supportAreas && (
                                <span>
                                  Support Areas: {client.profileData.supportAreas.join(", ")}
                                </span>
                              )}
                            </div>
                          )}
              ...
              <CardContent>
                <div className="space-y-4">
                  {data.recentActivity.map((activity: any) => (
                    <div
                      key={activity.id}
                      className="flex items-center space-x-4 p-3 hover:bg-hive-light-blue/20 rounded-lg"
                    >
                      <div
                      ...
                      >
                        ...
                        {activity.type === "message_received" && (
                          <MessageSquare className="h-4 w-4 text-hive-blue" />
                        )}
                        {activity.type === "note_added" && (
                          <FileText className="h-4 w-4 text-hive-purple" />
                        )}
                      </div>
                      <div className="flex-1">
                        <p
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {appointmentToCancel
                  ? ` with ${appointmentToCancel.clientName} on ${appointmentToCancel.date} at ${appointmentToCancel.time}`
                  : ""}
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {appointmentToReschedule ? ` with ${appointmentToReschedule.clientName}` : ""}. No
                additional charge will be applied to your client.
              </DialogDescription>
            </DialogHeader>
