...
import { Users, Video, Shield, Mail, CheckCircle, Calendar, UserPlus, Archive, RotateCcw, AlertTriangle, MessageCircle, ThumbsUp, ThumbsDown, RefreshCw, Eye, FileText, Download, Calendar as CalendarIcon, CheckSquare, Clock, Settings } from "lucide-react";
...
export default function AdminConsole({ user }: AdminConsoleProps) {
  ...
  const { data: chatbotConversations, isLoading: isChatbotLoading, refetch: refetchConversations } = useQuery({
  ...
  const { data: conversationDetails, isLoading: isConversationDetailsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest('GET', `/api/admin/chatbot/conversations/details/${selectedConversation.sessionId}`);
  ...
  const suspendUserMutation = useMutation({
    mutationFn: async ({ userId, reason, suspensionDuration }: { userId: string; reason: string; suspensionDuration?: string }) => {
      return await apiRequest('POST', `/api/admin/user/${userId}/suspend`, { reason, suspensionDuration });
    },
    onSuccess: (data) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="bg-white border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-secondary font-medium text-hive-black/70">Active Users</p>
                    <p className="text-2xl font-primary font-bold text-hive-black">{stats.activeUsers.toLocaleString()}</p>
            ...
            <Card className="bg-white border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-secondary font-medium text-hive-black/70">Sessions Today</p>
                    <p className="text-2xl font-primary font-bold text-hive-black">{stats.sessionsToday}</p>
            ...
            <Card className="bg-white border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-secondary font-medium text-hive-black/70">Email Queue</p>
                    <p className="text-2xl font-primary font-bold text-hive-black">{stats.emailQueue}</p>
            ...
            <Card className="bg-white border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-secondary font-medium text-hive-black/70">System Status</p>
                    <p className="text-2xl font-primary font-bold text-green-600">Secure</p>
                    <p className="text-xs font-secondary text-green-600">all systems operational</p>
                  </div>
            ...
            <Card className="bg-white border-gray-200 shadow-md">
              ...
              <CardContent>
                <div className="space-y-4">
                  <p className="text-sm text-hive-black/70 font-secondary">
                    Archive inactive client records with GDPR compliance. Archived records are retained for 7 years as required by law.
                  </p>
                    ...
                    {demoClients.map((client) => (
                      <div key={client.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                          ...
                          <p className="text-xs text-hive-black/50">Last session: {client.lastSession}</p>
                        </div>
            ...
            <Card className="bg-white border-gray-200 shadow-md">
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-hive-black">
                  ...
                  >
                    <RefreshCw className={`h-4 w-4 mr-1 ${isChatbotLoading ? 'animate-spin' : ''}`} />
                          ...
                          <div key={conversation.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div className="flex-1">
                              <p className="font-medium text-hive-black">
                                {conversation.userMessage ? conversation.userMessage.substring(0, 50) + '...' : 'New conversation'}
                              </p>
                              <p className="text-sm text-hive-black/60">
                                {conversation.userName || 'Anonymous'} • {new Date(conversation.createdAt).toLocaleDateString()}
                              </p>
                            </div>
                            <div className="flex items-center space-x-2">
                              <Button 
            ...
            <Card className="bg-white border-gray-200 shadow-md">
              ...
              <CardContent>
                {showAvailabilitySettings ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-hive-black/70 font-secondary">
                      Configure your working hours, blocked dates, and calendar integration. Clients and therapists will only see available time slots.
                    </p>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                        <div>
                          <p className="font-medium text-hive-black">Weekly Schedule</p>
                          <p className="text-sm text-hive-black/60">Monday-Friday, 9:00 AM - 5:00 PM</p>
                      ...
                      <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-hive-black/60">50 minutes with 10-minute buffer</p>
                      ...
                      <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-hive-black/60">Christmas & New Year holidays</p>
        ...
        <Card className="service-card bg-hive-white">
          ...
          <CardContent>
            {isArchivedLoading ? (
              ...
              <div className="space-y-4">
                {archivedClients && Array.isArray(archivedClients) && archivedClients.length > 0 ? (
                    ...
                    <div key={client.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Retention expires: {new Date(client.dataRetentionExpiry).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="flex items-center space-x-2">
      ...
      <Card className="service-card bg-hive-white">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              <Button 
                ...
                disabled={isChatbotLoading}
              >
                <RefreshCw className={`w-4 h-4 mr-1 ${isChatbotLoading ? 'animate-spin' : ''}`} />
                Refresh
              </Button>
            </div>
          </div>
        ...
        <CardContent>
          ...
          ) : chatbotConversations && Array.isArray(chatbotConversations) && chatbotConversations.length > 0 ? (
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {chatbotConversations.map((conversation: any, index: number) => (
                <div key={conversation.id || index} className="border border-gray-200 rounded-lg p-4 space-y-3">
                  {/* Conversation Header */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <Badge variant="outline">
                        {conversation.responseSource || 'ai'}
                      </Badge>
                    ...
                    <div className="text-xs text-gray-500">
                      {conversation.createdAt ? new Date(conversation.createdAt).toLocaleString() : 'Recent'}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-2 bg-gray-50 rounded text-sm">
                      ...
                      <p className="text-gray-700 mt-1">
                        {conversation.aiResponse ? conversation.aiResponse.substring(0, 150) + '...' : 'Processing...'}
                      </p>
                    </div>
                  </div>
      ...
      <Dialog open={isArchiveDialogOpen} onOpenChange={setIsArchiveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently archive {selectedArchiveClient?.name}'s account and remove their access to the platform.
              All data will be retained for 7 years as required by law.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
      ...
      <Dialog open={isSuspendDialogOpen} onOpenChange={setIsSuspendDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will temporarily suspend {selectedSuspendUser?.name}'s account and remove their platform access.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmSuspend} disabled={suspendUserMutation.isPending} variant="destructive">
      ...
      <Dialog open={isTerminateDialogOpen} onOpenChange={setIsTerminateDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmTerminate} disabled={terminateSessionMutation.isPending} variant="destructive">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-hive-black">{selectedConversation.userName || 'Anonymous'}</p>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Started:</span>
                    <p className="text-hive-black">{new Date(selectedConversation.createdAt).toLocaleString()}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-hive-black">{conversationDetails?.messages?.length || 'Loading...'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-hive-black">{selectedConversation.ipAddress || 'Not tracked'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button 
                    ...
                    onClick={() => {
                      setConversationActions(prev => ({ ...prev, helpful: !prev.helpful, unhelpful: false }));
                      toast({
                        title: conversationActions.helpful ? "Removed helpful rating" : "Marked as helpful",
                        description: "Conversation feedback updated successfully."
                      });
                    }}
                  >
                    <ThumbsUp className={`h-4 w-4 mr-1 ${conversationActions.helpful ? 'text-white' : ''}`} />
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      setConversationActions(prev => ({ ...prev, unhelpful: !prev.unhelpful, helpful: false }));
                      toast({
                        title: conversationActions.unhelpful ? "Removed unhelpful rating" : "Marked as unhelpful",
                        description: "Conversation feedback updated successfully."
                      });
                    }}
                  >
                    <ThumbsDown className={`h-4 w-4 mr-1 ${conversationActions.unhelpful ? 'text-white' : ''}`} />
                  ...
                  <Button 
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const response = await fetch(`/api/admin/chatbot/conversations/export-text/${selectedConversation.id}`, {
                          method: 'GET',
                          credentials: 'include',
                        });
                        ...
                        toast({
                          ...
                          description: "Secure conversation export downloaded successfully (antivirus-safe format)."
                        });
                      } catch (error) {
                  ...
                  <Button 
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const response = await fetch(`/api/admin/chatbot/conversations/pdf/${selectedConversation.id}`, {
                          method: 'GET',
                          credentials: 'include',
                        });
                        ...
                        toast({
                          ...
                          description: "Medical-grade secure PDF downloaded with enhanced security headers."
                        });
                      } catch (error) {
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      toast({
                        title: conversationActions.flagged ? "Removed review flag" : "Flagged for review",
                        description: conversationActions.flagged ? "Conversation no longer flagged for manual review." : "Conversation flagged for manual review by therapy oversight team."
                      });
                    }}
                  >
                    <AlertTriangle className={`h-4 w-4 mr-1 ${conversationActions.flagged ? 'text-white' : ''}`} />
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={bulkExportType} onValueChange={(value: 'selected' | 'dateRange') => setBulkExportType(value)}>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="selected" id="selected" />
                  <label htmlFor="selected" className="text-sm cursor-pointer">Select Individual Conversations</label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="dateRange" id="dateRange" />
                  <label htmlFor="dateRange" className="text-sm cursor-pointer">Export by Date Range</label>
                </div>
              </RadioGroup>
            </div>
            ...
            {bulkExportType === 'selected' && (
              <div className="space-y-3">
                <Label className="text-sm font-medium">Select Conversations ({selectedConversationIds.length} selected)</Label>
                    ...
                    <div key={conv.id} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setSelectedConversationIds(prev => prev.filter(id => id !== conv.id));
                          }
                        }}
                      />
                      <label htmlFor={conv.id} className="text-sm cursor-pointer flex-1">
                        <div className="flex justify-between">
                          ...
                          <span className="text-gray-500">{new Date(conv.createdAt).toLocaleDateString()}</span>
                        </div>
                      </label>
                    </div>
                  ))}
                ...
                <div className="flex justify-between">
                  <Button 
                    variant="outline" 
                    size="sm" 
                    onClick={() => setSelectedConversationIds((chatbotConversations as any[])?.map((conv: any) => conv.id) || [])}
                  >
                    Select All
                  </Button>
                  <Button 
