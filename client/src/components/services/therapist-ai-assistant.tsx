...
export default function TherapistAiAssistant({ user }: TherapistAiAssistantProps) {
  ...
  const sendChatMessageMutation = useMutation({
    mutationFn: async (data: { message: string; userMessage: AiChatMessage }) => {
      const res = await apiRequest('POST', '/api/therapist/ai/chat', { message: data.message, context: selectedClient });
    ...
    onError: (error: any) => {
      ...
      if (error.message?.includes('Privacy violation detected')) {
        ...
        toast({
          title: "Chat Failed",
          description: "Unable to send message to Chat assistant. Please ensure professional language is used.",
          variant: "destructive",
        });
      }
    },
        ...
        <head>
          ...
          <style>
            ...
            .severity-badge { 
              ...
              border-radius: 20px; 
              font-weight: bold;
              ${plan.severity === 'mild' ? 'background: #d4edda; color: #155724;' : 
                plan.severity === 'moderate' ? 'background: #fff3cd; color: #856404;' :
  ...
  const demoSessionAnalyses: SessionAnalysis[] = [
    {
      ...
      sessionNotes: 'Client presented with moderate anxiety levels. Demonstrated good insight into triggers.',
      actionItems: ['Practice mindfulness daily', 'Complete thought record', 'Sleep diary for one week']
    },
    {
      sessionId: 'session-2',
      ...
      sessionNotes: 'Notable improvement in mood and energy. Client completing homework assignments.',
      actionItems: ['Schedule one social activity', 'Continue mood tracking', 'Practice positive self-talk']
    }
  ];
  ...
  const demoTherapyRecommendations: TherapyRecommendation[] = [
    {
      ...
      aiRecommendations: {
        ...
        techniques: ['Cognitive restructuring', 'Exposure therapy', 'Mindfulness-based interventions'],
        sessionFrequency: 'Weekly for 12-16 sessions',
        expectedOutcomes: ['Reduced anxiety symptoms', 'Improved coping skills', 'Better sleep quality'],
        timeframe: '3-4 months',
        additionalResources: ['CBT workbook', 'Meditation app', 'Anxiety support group'],
        warningSignsToWatch: ['Panic attacks', 'Avoidance escalation', 'Sleep disturbance worsening']
      },
  ...
  const displayRecommendations = treatmentRecommendations.length > 0 ? treatmentRecommendations : demoTherapyRecommendations;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3 className="font-medium text-yellow-800 mb-2">⚠️ Important: AI Clinical Support Guidelines</h3>
            <div className="space-y-1 text-sm text-yellow-700">
              <p><strong>Privacy:</strong> This tool is not for sharing confidential or identifying personal health information. All interactions are logged and monitored.</p>
              <p><strong>AI Limitations:</strong> All AI recommendations and analyses require professional clinical validation and must be reviewed by qualified therapists before implementation.</p>
              <p><strong>Professional Judgement:</strong> AI suggestions are for reference only and do not replace clinical expertise, professional assessment, or therapeutic judgement.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-century font-bold text-hive-black">Chat Therapy Assistant</h2>
          <p className="text-gray-600 mt-2">Chat-powered insights and recommendations for your therapy practice</p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-900">
                  {displayAnalyses.length > 0 ? Math.round(displayAnalyses.reduce((acc, a) => acc + (a.aiInsights?.confidenceScore || 0), 0) / displayAnalyses.length) : 0}%
                </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-900">{displayRecommendations.length}</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-900">
                  {displayInsights.length > 0 ? Math.round(displayInsights.reduce((acc, i) => acc + (i.overallProgress || 0), 0) / displayInsights.length) : 0}%
                </p>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-lg font-semibold text-green-900">{newAnalysisResults.clientName}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-green-700">Session Duration</p>
                <p className="text-lg font-semibold text-green-900">{newAnalysisResults.duration} minutes</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-gray-800">{newAnalysisResults.aiInsights?.emotionalState || 'Not available'}</p>
              </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-green-200">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-900">{newAnalysisResults.aiInsights?.confidenceScore || 0}%</p>
                </div>
                <Button 
                  variant="outline" 
                  size="sm"
              ...
              <Card key={analysis.sessionId} className="border-2">
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-700">{analysis.sessionNotes || 'No session notes available'}</p>
                  </div>
                </CardContent>
              </Card>
            ))}
            ...
            <Card className="border-2 border-purple-500 bg-purple-50">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-semibold text-lg text-purple-900">{generatedPlan.clientName}</h4>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-purple-800 bg-white p-3 rounded border">
                        {generatedPlan.aiRecommendations?.primaryApproach || 'Approach details will be available shortly'}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-purple-800 bg-white p-3 rounded border">
                        {generatedPlan.aiRecommendations?.sessionFrequency || 'Frequency details will be available shortly'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {(generatedPlan.aiRecommendations?.techniques || []).slice(0, 4).map((technique, idx) => (
                        <div key={idx} className="flex items-center gap-2 text-sm text-purple-800 bg-white p-2 rounded border">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <div className="text-sm text-gray-500">
                        Updated: {recommendation.lastUpdated && !isNaN(new Date(recommendation.lastUpdated).getTime())
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm font-medium text-blue-700">
                          {recommendation.aiRecommendations?.primaryApproach || 'Approach details available'}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {(recommendation.aiRecommendations?.techniques || []).map((technique, idx) => (
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-700">
                          <strong>Frequency:</strong> {recommendation.aiRecommendations?.sessionFrequency || 'To be determined'}
                        </p>
                        <p className="text-sm text-gray-700">
                          <strong>Timeframe:</strong> {recommendation.aiRecommendations?.timeframe || 'To be determined'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <ul className="text-sm space-y-1">
                          {(recommendation.aiRecommendations?.expectedOutcomes || []).map((outcome, idx) => (
                            <li key={idx} className="flex items-center gap-2">
                              <CheckCircle className="w-4 h-4 text-green-600" />
                              {outcome}
                            </li>
                      ...
                      <div>
                        ...
                        <ul className="text-sm space-y-1">
                          {(recommendation.aiRecommendations?.warningSignsToWatch || []).map((sign, idx) => (
                            <li key={idx} className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <ul className="text-sm space-y-1">
                          {(recommendation.aiRecommendations?.additionalResources || []).map((resource, idx) => (
                            <li key={idx} className="flex items-center gap-2">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <Badge className={
                          insight.trendDirection === 'improving' ? 'bg-green-100 text-green-800' :
                          insight.trendDirection === 'stable' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <div className="text-right text-sm text-gray-500">
                      Last Assessment: {insight.lastAssessment && !isNaN(new Date(insight.lastAssessment).getTime())
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <div className="flex items-start space-x-2">
              ...
              <p className="text-sm text-yellow-800">
                <strong>Privacy Reminder:</strong> This tool is not for sharing confidential or identifying personal health information. 
                Use anonymised scenarios and general clinical questions only.
              </p>
            </div>
          </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {['Anxiety Disorders', 'Depression', 'ADHD', 'Trauma/PTSD'].map((condition, idx) => (
              ...
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800">
                    Consider incorporating more structured homework assignments. Clients with regular homework completion show 23% better outcomes.
                  </p>
                </div>
                <div className="p-4 bg-green-50 rounded-lg">
                  <h4 className="font-medium text-green-900 mb-2">Session Scheduling</h4>
                  <p className="text-sm text-green-800">
                    Evening sessions (6-8 PM) show highest attendance rates (96%) and client engagement scores.
                  </p>
                </div>
                <div className="p-4 bg-purple-50 rounded-lg">
                  <h4 className="font-medium text-purple-900 mb-2">Professional Development</h4>
                  <p className="text-sm text-purple-800">
                    Based on your client demographics, consider training in adolescent DBT techniques for improved outcomes with younger clients.
                  </p>
                </div>
              </div>
            </CardContent>
