...
export default function AppointmentManagement() {
  ...
  const { data: appointmentsData, isLoading, error } = useQuery<Appointment[]>({
  ...
  const filteredAppointments = appointments.filter(appointment => {
    ...
    const matchesTimeFilter = 
      archiveView === 'upcoming' ? !isPast && !appointment.isArchived :
      archiveView === 'past' ? isPast && !appointment.isArchived :
      archiveView === 'archived' ? appointment.isArchived :
      true;
  ...
  const handleSelectAllForArchive = (checked: boolean) => {
    if (checked) {
      const eligibleAppointments = filteredAppointments.filter(a => 
        !a.isArchived && (a.status === 'completed' || a.status === 'cancelled' || a.status === 'no_show')
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-hive-black/60 font-secondary">Monitor and manage all therapy appointments</p>
        </div>
        <Button 
            ...
            <Card className="bg-white border-0 shadow-md">
              <CardHeader>
                <CardTitle className="font-primary text-hive-black">
                  Upcoming Appointments ({filteredAppointments.filter(a => {
                    const now = new Date();
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="text-left p-4 font-secondary text-hive-black/70">Client</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Therapist</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Date & Time</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Status</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Duration</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Fee</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredAppointments.filter(a => {
                          ...
                          <tr key={appointment.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="p-4">
                              <span className="font-secondary text-sm text-hive-black/80 font-mono" data-testid={`appointment-id-${appointment.id}`}>
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-secondary text-hive-black" data-testid={`appointment-client-${appointment.id}`}>
                                  {appointment.clientName}
                                </span>
                              </div>
                            </td>
                            <td className="p-4">
                              <span className="font-secondary text-hive-black" data-testid={`appointment-therapist-${appointment.id}`}>
                                {appointment.therapistName}
                              </span>
                            </td>
                            <td className="p-4">
                              <div className="space-y-1">
                                <p className="font-secondary text-hive-black text-sm" data-testid={`appointment-date-${appointment.id}`}>
                                  {appointment.date ? new Date(appointment.date).toLocaleDateString('en-GB') : 
                                   appointment.scheduledAt ? new Date(appointment.scheduledAt).toLocaleDateString('en-GB') : 'TBD'}
                                </p>
                                <p className="font-secondary text-hive-black/60 text-xs" data-testid={`appointment-time-${appointment.id}`}>
                                  {appointment.time || 
                                   (appointment.scheduledAt ? new Date(appointment.scheduledAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) : 'TBD')}
                                </p>
                              </div>
                            </td>
                            <td className="p-4">
                              <Badge className={`${getStatusColor(appointment.status)} flex items-center gap-1 w-fit`} data-testid={`appointment-status-${appointment.id}`}>
                                {getStatusIcon(appointment.status)}
                                {appointment.status.charAt(0).toUpperCase() + appointment.status.slice(1).replace('_', ' ')}
                              </Badge>
                            </td>
                            <td className="p-4">
                              <span className="font-secondary text-hive-black/80" data-testid={`appointment-duration-${appointment.id}`}>
                                {appointment.duration} min
                              </span>
                            </td>
                            <td className="p-4">
                              <span className="font-secondary text-hive-black font-medium" data-testid={`appointment-fee-${appointment.id}`}>
                                £{appointment.fee}
                              </span>
                            </td>
                            <td className="p-4">
        ...
        <TabsContent value="past">
          <div className="space-y-6">
            ...
            <Card className="bg-white border-0 shadow-md">
              <CardHeader>
                <CardTitle className="font-primary text-hive-black">
                  Past Appointments ({filteredAppointments.filter(a => {
                    const now = new Date();
                    const appointmentTime = new Date(a.scheduledAt || a.date);
                    return appointmentTime < now && !a.isArchived;
                  }).length})
                      ...
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="text-left p-4 font-secondary text-hive-black/70 w-10">
                            <Checkbox
                              checked={selectedForArchive.size === filteredAppointments.filter(a => {
                                const now = new Date();
                                const appointmentTime = new Date(a.scheduledAt || a.date);
                                return appointmentTime < now && !a.isArchived && canArchiveAppointment(a);
                              }).length && filteredAppointments.filter(a => {
                                const now = new Date();
                                const appointmentTime = new Date(a.scheduledAt || a.date);
                                return appointmentTime < now && !a.isArchived && canArchiveAppointment(a);
                              }).length > 0}
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="text-left p-4 font-secondary text-hive-black/70">Duration</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Fee</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Archive Info</th>
                          <th className="text-left p-4 font-secondary text-hive-black/70">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredAppointments.filter(a => a.isArchived).map((appointment) => (
                          <tr key={appointment.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="p-4">
                              <span className="font-secondary text-sm text-hive-black/80 font-mono" data-testid={`appointment-id-${appointment.id}`}>
                                {appointment.id.substring(0, 8)}...
                            ...
                            <td className="p-4">
                              <div className="space-y-1">
                                <p className="text-xs text-gray-500">
                                  {appointment.archivedAt ? `Archived ${formatDistance(new Date(appointment.archivedAt), new Date(), { addSuffix: true })}` : 'N/A'}
                                </p>
                                <p className="text-xs text-gray-500">
                                  By: {appointment.archivedBy || 'Unknown'}
                                </p>
                                {appointment.archivedReason && (
                                  <p className="text-xs text-gray-500" title={appointment.archivedReason}>
                                    Reason: {appointment.archivedReason.length > 30 ? 
