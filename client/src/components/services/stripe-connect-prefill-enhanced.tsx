...
export default function StripeConnectPrefillEnhanced({ therapistId }: StripeConnectPrefillEnhancedProps) {
  const [currentStep, setCurrentStep] = useState(1);
  const [applicationData, setApplicationData] = useState<Partial<StripeConnectApplication['applicationData']>>({
  ...
  const refreshStatusMutation = useMutation({
    mutationFn: () => 
      apiRequest('GET', `/api/automation/stripe-connect/status/${therapistId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/automation/stripe-connect'] });
    }
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Complete your Stripe Connect onboarding to receive payments</p>
        </div>
      ...
      {!existingApplication || existingApplication.accountStatus === 'pending' ? (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-gray-600">
                Automatically pre-fill your Stripe Connect application using your therapist profile information.
              </p>
              
              <Alert>
                <CheckCircle className="w-4 h-4" />
                <AlertDescription>
                  This will create a Stripe Express account and pre-fill your personal details, business type (Individual), and UK address information.
                </AlertDescription>
              </Alert>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {existingApplication.onboardingUrl && !existingApplication.verificationStatus.payoutsEnabled && (
              <div className="mt-4">
                <Alert>
                  <AlertTriangle className="w-4 h-4" />
                  <AlertDescription>
                    Your account setup is incomplete. Complete the onboarding process to start receiving payments.
                  </AlertDescription>
                </Alert>
            ...
            {existingApplication.verificationStatus.payoutsEnabled && (
              <Alert>
                <CheckCircle className="w-4 h-4" />
                <AlertDescription>
                  Congratulations! Your Stripe Connect account is fully active. You can now receive payments from therapy sessions.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
