...
export default function VideoSessionsProduction({ user, onBackToDashboard, sessionId }: VideoSessionsProductionProps) {
  ...
  const { data: sessions = [], isLoading, error } = useQuery<VideoSession[]>({
  ...
  const getSessionStatus = (session: VideoSession) => {
    ...
    const endTime = session.endTime ? new Date(session.endTime) : new Date(scheduledAt.getTime() + session.duration * 60000);
  ...
  const renderJoinButton = (session: VideoSession) => {
    ...
    const buttonText = status === 'in-progress' ? 'Join Now' : 
                      status === 'starting-soon' ? 'Join Session' : 'Join Session';
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-400" data-testid={`text-counterpart-${session.id}`}>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-sm font-medium" data-testid={`text-date-${session.id}`}>{date}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400" data-testid={`text-time-${session.id}`}>{time}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400" data-testid={`text-duration-${session.id}`}>
