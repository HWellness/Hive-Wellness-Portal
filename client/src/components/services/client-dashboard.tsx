...
import { Calendar, Clock, Video, Heart, Brain, TrendingUp, MessageCircle, CheckCircle, Activity, User as UserIcon, CreditCard, LogOut, Settings, RefreshCw, X, AlertTriangle } from "lucide-react";
...
export default function ClientDashboard({ user, onNavigateToService }: ClientDashboardProps) {
  ...
  const { data: dashboardData, isLoading, error } = useQuery<ClientDashboardData>({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-primary font-bold text-hive-purple">{data.completedSessions}</p>
                  </div>
                  <div className="bg-hive-purple/10 p-3 rounded-xl">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-primary font-bold text-hive-purple">{data.wellnessMetrics.overallProgress}%</p>
                    <Progress value={data.wellnessMetrics.overallProgress} className="w-full mt-2 h-2" />
                  </div>
                  <div className="bg-hive-purple/10 p-3 rounded-xl">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-primary font-bold text-hive-purple">{data.unreadMessages}</p>
              ...
              {data.nextAppointment ? (
                        ...
                        {data.nextAppointment.therapistProfileImage ? (
                          <img 
                            ...
                            onError={(e) => {
                              ...
                              e.currentTarget.parentNode?.querySelector('.therapist-fallback')?.classList.remove('hidden');
                            }}
                          />
                        ) : null}
                        <div className={`w-full h-full bg-hive-purple/10 flex items-center justify-center therapist-fallback ${data.nextAppointment.therapistProfileImage ? 'hidden' : ''}`}>
                          <UserIcon className="w-6 h-6 text-hive-purple" />
                        </div>
                      </div>
                      <div>
                        <h3 className="font-semibold text-hive-black">{data.nextAppointment.therapistName}</h3>
                        <p className="text-sm text-hive-black">{data.nextAppointment.type}</p>
                        <p className="text-sm text-hive-black">{data.nextAppointment.date} at {data.nextAppointment.time}</p>
                      </div>
                    </div>
                    <Badge className="bg-purple-100 text-hive-purple">
                      {data.nextAppointment.status === 'confirmed' ? 'Confirmed' : 
                       data.nextAppointment.status === 'in_progress' ? 'In Progress' : 
                       data.nextAppointment.status === 'completed' ? 'Completed' : 'Scheduled'}
                    </Badge>
                  </div>
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-hive-black mb-4">
                    You're assigned to {data.assignedTherapist.firstName} {data.assignedTherapist.lastName}. Ready to book your next session!
                  </p>
                  <Button 
                    className="mt-4 bg-hive-purple text-white hover:bg-hive-purple/90"
                    onClick={() => setSelectedService('scheduling')}
                  >
                    Book Session with {data.assignedTherapist.firstName} {data.assignedTherapist.lastName}
                  </Button>
                </div>
              ) : (
                <div className="text-center py-6">
                  ...
                  <Button variant="outline" className="mt-4 border-hive-purple text-hive-purple" disabled>
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between text-sm">
                ...
                <span className="text-hive-purple font-semibold">{data.wellnessMetrics.moodScore}/10</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm">
                ...
                <span className="text-hive-purple font-semibold">{data.wellnessMetrics.sleepQuality}/10</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm">
                ...
                <span className="text-hive-purple font-semibold">{data.wellnessMetrics.stressLevel}/10</span>
              </div>
            ...
            <Dialog open={isWellnessDialogOpen} onOpenChange={setIsWellnessDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  <DialogTitle className="text-hive-purple font-primary">Update Your Wellness Metrics</DialogTitle>
                </DialogHeader>
                <div className="space-y-6 py-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="sleep-quality" className="text-sm font-medium text-hive-black">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="stress-level" className="text-sm font-medium text-hive-black">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="wellness-notes" className="text-sm font-medium text-hive-black">
                      Additional Notes
                    </Label>
                    <Textarea
                      id="wellness-notes"
            ...
            {data.therapeuticGoals.map((goal, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm text-hive-purple font-semibold">{goal.progress}%</span>
                </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {data.recentSessions.map((session, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-purple-100 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white shadow-sm">
                      ...
                      <div className={`w-full h-full bg-hive-purple/10 flex items-center justify-center therapist-fallback ${session.therapistProfileImage ? 'hidden' : ''}`}>
                        <UserIcon className="w-5 h-5 text-hive-purple" />
                      </div>
                    </div>
                    <div>
                      <p className="font-medium text-hive-black">{session.therapistName}</p>
                      <p className="text-sm text-hive-black">{session.date} • {session.duration}</p>
                    </div>
                  </div>
                  <Badge className="bg-purple-100 text-hive-purple">
                    {session.status}
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8" key="enhanced-services-grid">
            {clientServices.map((service) => {
              const IconComponent = service.icon === 'Home' ? Activity :
                service.icon === 'User' ? UserIcon :
                service.icon === 'Calendar' ? Calendar :
                service.icon === 'Video' ? Video :
                service.icon === 'MessageSquare' ? MessageCircle :
                service.icon === 'CreditCard' ? CreditCard :
                service.icon === 'TrendingUp' ? TrendingUp : Activity;
              ...
              return (
                <div
                  ...
                  onClick={() => isBookingService && !hasAssignedTherapist ? 
                    toast({
                      title: "Therapist Assignment Pending",
                      description: "Our admin team will assign you a therapist shortly. You'll be notified once your therapist is ready.",
                      variant: "default"
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {isBookingService && !hasAssignedTherapist && (
                          <Badge variant="outline" className="ml-2 text-xs bg-hive-purple/10 text-hive-purple border-hive-purple/20 animate-pulse">
                            Awaiting Assignment
                          </Badge>
                        )}
                      </div>
      ...
      <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to cancel your appointment{appointmentToCancel ? ` with ${appointmentToCancel.therapistName} on ${appointmentToCancel.date} at ${appointmentToCancel.time}` : ''}? 
              This action cannot be undone. Please note our cancellation policy may apply depending on when you cancel.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showRescheduleDialog} onOpenChange={setShowRescheduleDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a new date and time for your appointment{appointmentToReschedule ? ` with ${appointmentToReschedule.therapistName}` : ''}.
