...
const SPECIALISATIONS = [
  ...
  'Addiction & Substance Abuse', 'Eating Disorders', 'Grief & Loss', 'Stress Management',
  'OCD', 'Bipolar Disorder', 'Personality Disorders', 'ADHD', 'Autism Spectrum',
  'Family Therapy', 'Couples Therapy', 'Child & Adolescent', 'LGBTQ+ Issues',
  'Career Counselling', 'Life Transitions', 'Anger Management'
];

const THERAPEUTIC_APPROACHES = [
  'Cognitive Behavioural Therapy (CBT)', 'Psychodynamic Therapy', 'Humanistic Therapy',
  'EMDR', 'Dialectical Behaviour Therapy (DBT)', 'Acceptance & Commitment Therapy (ACT)',
  'Mindfulness-Based Therapy', 'Solution-Focused Therapy', 'Systemic Family Therapy',
  'Gestalt Therapy', 'Person-Centred Therapy', 'Narrative Therapy', 'Schema Therapy'
];
...
export default function TherapistProfileCompletion({ user }: TherapistProfileCompletionProps) {
      ...
      if (data.success && data.hasData) {
        ...
        setProfileData(prevData => {
          ...
          const newData = {
            ...
            professionalInfo: {
              ...
              qualifications: Array.isArray(hubspotData.qualifications) ? hubspotData.qualifications : 
                             hubspotData.qualifications ? [hubspotData.qualifications] : 
                             hubspotData.highestQualification ? [hubspotData.highestQualification] : 
                             prevData.professionalInfo.qualifications,
              yearsExperience: hubspotData.experience || prevData.professionalInfo.yearsExperience,
              specialisations: hubspotData.specializations && hubspotData.specializations.length > 0 ? hubspotData.specializations : prevData.professionalInfo.specialisations,
              therapeuticApproach: hubspotData.specializations && hubspotData.specializations.length > 0 ? hubspotData.specializations : prevData.professionalInfo.therapeuticApproach,
              registrationBody: hubspotData.professionalBody || prevData.professionalInfo.registrationBody,
            },
            businessInfo: {
          ...
          if (hubspotData.firstName && hubspotData.firstName !== prevData.personalInfo.firstName) changedFields.push('First Name');
          if (hubspotData.lastName && hubspotData.lastName !== prevData.personalInfo.lastName) changedFields.push('Last Name');
          if (hubspotData.phone && hubspotData.phone !== prevData.personalInfo.phone) changedFields.push('Phone');
          if (hubspotData.location && hubspotData.location !== prevData.personalInfo.address) changedFields.push('Address');
          if (hubspotData.experience && hubspotData.experience !== prevData.professionalInfo.yearsExperience) changedFields.push('Experience');
          if (hubspotData.qualifications && hubspotData.qualifications !== prevData.professionalInfo.qualifications.join(',')) changedFields.push('Qualifications');
          if (hubspotData.specializations && JSON.stringify(hubspotData.specializations) !== JSON.stringify(prevData.professionalInfo.specialisations)) changedFields.push('Specialisations');
          if (hubspotData.professionalBody && hubspotData.professionalBody !== prevData.professionalInfo.registrationBody) changedFields.push('Professional Body');
        ...
        if (changedFields.length > 0) {
          ...
          toast({
            ...
            description: "Your profile already contains all the available information from your application. No changes were needed.",
          });
        }
      } else {
        toast({
          title: "No Additional Application Data Found",
          description: data.message || "No additional application data was found for auto-population.",
          variant: "destructive",
        });
      }
    },
  ...
  const getInitialProfileData = (): TherapistProfileData => {
    const application = (existingApplication as any)?.exists ? (existingApplication as any).application : null;
    ...
    return {
      ...
      professionalInfo: {
        ...
        yearsExperience: application?.experience || application?.yearsOfExperience?.toString() || '',
  ...
  React.useEffect(() => {
    if ((existingApplication as any)?.exists && (existingApplication as any).application) {
      ...
      setProfileData(prevData => ({
        ...
        professionalInfo: {
          ...prevData.professionalInfo,
          specialisations: app.specializations || app.therapy_specialisations || app.specialisations || [],
          registrationBody: app.professionalBody || app.professional_body || prevData.professionalInfo.registrationBody,
          qualifications: app.qualifications ? (Array.isArray(app.qualifications) ? app.qualifications : [app.qualifications]) : 
                         app.highestQualification ? [app.highestQualification] : [],
          yearsExperience: app.experience || app.yearsOfExperience?.toString() || prevData.professionalInfo.yearsExperience,
      ...
      toast({
        ...
        description: "Your profile has been pre-filled with information from your initial application. Please review and complete any missing details.",
      });
    }
  ...
  const sections = [
    ...
    { id: 'professional', title: 'Professional Qualifications', icon: GraduationCap, color: 'bg-green-500' },
  ...
  const handleCompleteProfile = () => {
    ...
    if (!profileData.privacy.dataProcessingConsent || !profileData.privacy.professionalStandardsConsent || !profileData.privacy.backgroundCheckConsent) {
      toast({
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <h3 className="text-lg font-semibold text-hive-purple">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-century font-bold text-hive-black">Complete Your Professional Profile</h2>
          <p className="text-gray-600 mt-2">Help us verify your qualifications and match you with suitable clients</p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setProfileData(prev => ({
                      ...prev,
                      personalInfo: { ...prev.personalInfo, emergencyPhone: formatPhoneNumber(e.target.value) }
                    }))}
                    placeholder="+44 7XXX XXXXXX"
                  />
                </div>
            ...
            <div className="space-y-6">
                    ...
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, qualifications: newQuals }
                          }));
                        }}
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const newQuals = profileData.professionalInfo.qualifications.filter((_, i) => i !== index);
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, qualifications: newQuals }
                          }));
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setProfileData(prev => ({
                      ...prev,
                      professionalInfo: { ...prev.professionalInfo, registrationNumber: e.target.value }
                    }))}
                ...
                <div>
                  ...
                  <Select value={profileData.professionalInfo.registrationBody} onValueChange={(value) => 
                    setProfileData(prev => ({
                      ...prev,
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setProfileData(prev => ({
                    ...prev,
                    professionalInfo: { ...prev.professionalInfo, yearsExperience: e.target.value }
                  }))}
                  placeholder="e.g. 5"
                />
              </div>
                    ...
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, specialisations: newSpecs }
                          }));
                        }}
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const newSpecs = profileData.professionalInfo.specialisations.filter((_, i) => i !== index);
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, specialisations: newSpecs }
                          }));
                  ...
                  {profileData.professionalInfo.therapeuticApproach.map((approach, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        value={approach}
                        onChange={(e) => {
                          const newApproaches = [...profileData.professionalInfo.therapeuticApproach];
                          newApproaches[index] = e.target.value;
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, therapeuticApproach: newApproaches }
                          }));
                        }}
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const newApproaches = profileData.professionalInfo.therapeuticApproach.filter((_, i) => i !== index);
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, therapeuticApproach: newApproaches }
                          }));
                  ...
                  {profileData.professionalInfo.languagesSpoken.map((lang, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const newLangs = profileData.professionalInfo.languagesSpoken.filter((_, i) => i !== index);
                          setProfileData(prev => ({
                            ...prev,
                            professionalInfo: { ...prev.professionalInfo, languagesSpoken: newLangs }
                          }));
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={profileData.professionalInfo.professionalInsurance} onValueChange={(value) => 
                    setProfileData(prev => ({
                      ...prev,
                      professionalInfo: { ...prev.professionalInfo, professionalInsurance: value }
                    }))
                  }>
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setProfileData(prev => ({
                      ...prev,
                      professionalInfo: { ...prev.professionalInfo, insuranceProvider: e.target.value }
                    }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setProfileData(prev => ({
                    ...prev,
                    professionalInfo: { ...prev.professionalInfo, insuranceNumber: e.target.value }
                  }))}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setProfileData(prev => ({
                      ...prev,
                      businessInfo: { ...prev.businessInfo, bankSortCode: formatSortCode(e.target.value) }
                    }))}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setProfileData(prev => ({
                      ...prev,
                      businessInfo: { ...prev.businessInfo, businessPhone: formatPhoneNumber(e.target.value) }
                    }))}
                    placeholder="+44 7XXX XXXXXX"
                  />
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      I consent to Hive Wellness processing my personal data in accordance with their Privacy Policy for the purpose of providing therapy services and platform functionality.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg">
                  <input
                    ...
                    onChange={(e) => setProfileData(prev => ({
                      ...prev,
                      privacy: { ...prev.privacy, professionalStandardsConsent: e.target.checked }
                    }))}
                    className="mt-1"
                  />
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      I agree to maintain professional standards as outlined by my registration body and Hive Wellness's Terms of Service for therapists.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      I consent to Hive Wellness conducting necessary background checks and verifying my professional qualifications as required for therapist verification.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      I would like to receive updates about new features, training opportunities, and relevant news from Hive Wellness.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  <strong>Important:</strong> All required consent items must be checked to complete your profile and begin accepting clients through the Hive Wellness platform.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="flex gap-2">
              ...
              {currentSection === sections.length - 1 ? (
                <Button
                  ...
                  disabled={saveProfileMutation.isPending || !profileData.privacy.dataProcessingConsent || !profileData.privacy.professionalStandardsConsent || !profileData.privacy.backgroundCheckConsent}
