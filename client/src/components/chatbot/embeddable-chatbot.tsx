...
export default function EmbeddableChatbot({
        ...
        prev.map((msg) =>
          msg.id === newMessage.id
            ? {
                ...msg,
                response:
                  "I'm sorry, I'm having trouble connecting right now. Please try again or email support@hive-wellness.co.uk for assistance.",
              }
            : msg
        )
      );
  ...
  return (
    <div className={getPositionClasses()}>
      <Card
        className={`w-[400px] max-w-[calc(100vw-2rem)] shadow-2xl border-0 overflow-hidden transition-all duration-300 bg-white rounded-3xl ${compactMode ? "w-80" : ""} ${isMinimized ? "h-14" : "h-[550px]"}`}
      >
        <CardHeader
          className="pb-2 px-5 py-4 flex-shrink-0 rounded-t-3xl"
          style={{ backgroundColor: "var(--hive-purple)" }}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <CardTitle className="text-white text-lg font-medium">Hive Wellness</CardTitle>
            </div>
        ...
        {!isMinimized && (
          <CardContent
            className="p-0 flex flex-col bg-white"
