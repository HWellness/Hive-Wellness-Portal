...
import { Shield, ShieldCheck, ShieldX, Key, AlertCircle, Settings, Smartphone, Mail, Lock } from 'lucide-react';
...
export function MFASettings() {
  ...
  const generateBackupCodesMutation = useMutation({
    mutationFn: (password: string) => 
      apiRequest('POST', '/api/mfa/backup-codes', { password }),
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ['/api/mfa/status'] });
      ...
      toast({
        ...
        description: "Your new backup codes have been downloaded. The old codes are no longer valid.",
      });
    },
    onError: (error: any) => {
      toast({
      ...
      <Card data-testid="card-mfa-settings">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant={(mfaStatus?.enabled || mfaStatus?.mfaEnabled) ? "default" : "secondary"} data-testid="badge-mfa-status">
              {(mfaStatus?.enabled || mfaStatus?.mfaEnabled) ? 'Enabled' : 'Disabled'}
            </Badge>
          </div>

          {/* Available Methods Section */}
          {(mfaStatus?.enabled || mfaStatus?.mfaEnabled) && mfaStatus?.availableMethods && mfaStatus.availableMethods.length > 0 && (
          ...
          {(mfaStatus?.enabled || mfaStatus?.mfaEnabled) && (
            <div className="space-y-3 pt-4 border-t">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground" data-testid="text-backup-codes-count">
          ...
          {(mfaStatus?.setupAt || mfaStatus?.mfaSetupAt) && (
            <p className="text-xs text-muted-foreground">
              MFA was set up on {new Date(mfaStatus.setupAt || mfaStatus.mfaSetupAt!).toLocaleDateString()}
            </p>
          )}
        </CardContent>
      </Card>
