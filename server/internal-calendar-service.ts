...
import { toUtcFromUk, ukDayBoundsUtc, isValidUkLocalInstant, utcToUkTimeString } from './utils/time';
...
export class InternalCalendarService {
  ...
  async getAvailableSlots(ukDateStr: string): Promise<{
    ...
    if (!isWorkingDay) {
      console.log(`❌ ${ukDateStr} is not a working day (day ${dayOfWeek}), working days: ${availabilitySettings.workingDays.join(',')}`);
      return {
        slots: [],
        summary: {
          availableSlots: 0,
  ...
  async checkSlotAvailability(
    ...
    if (isNaN(slotStart.getTime()) || isNaN(slotEnd.getTime())) {
      console.error(`❌ Invalid date created: slotStart=${slotStart}, slotEnd=${slotEnd} for ${ukDateStr} ${time}`);
      return {
        isAvailable: false,
    ...
    try {
      ...
      if (isNaN(dayStart.getTime()) || isNaN(dayEnd.getTime())) {
        console.error(`❌ Invalid day bounds: dayStart=${dayStart}, dayEnd=${dayEnd} for ${ukDateStr}`);
        return {
          isAvailable: false,
      ...
      const existingCalls = await db.select()
      ...
      for (const call of existingCalls) {
        ...
        if (existingStart < slotEnd && existingEnd > slotStart) {
          console.log(`⚠️ Time conflict detected: Existing call ${existingStart.toISOString()} conflicts with slot ${slotStart.toISOString()}`);
          return {
            isAvailable: false,
      ...
      const blockingEvents = blocks.filter((block: any) => {
        ...
        const isAvailabilityWindow = title.includes('available') || title.includes('open') || title.includes('free');
  ...
  async reserveTimeSlot(bookingData: {
    ...
    const availability = await this.checkSlotAvailability(bookingData.ukDateStr, bookingData.time, availabilitySettings);
    ...
    console.log(`📅 Reserving time slot: ${slotStart.toISOString()} for ${bookingData.participantName}`);
    return { success: true };
  }
  ...
  async createCalendarBlock(blockData: {
    ...
    console.log(`📅 Created calendar block: ${blockData.title} (${blockData.startTime} - ${blockData.endTime})`);
    return blockId;
  }
