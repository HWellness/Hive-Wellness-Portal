...
export async function getTherapistEarnings(stripeAccountId: string): Promise<TherapistEarnings> {
  try {
    ...
    return {
      ...
      bankAccountConnected: account.external_accounts
    ...
    throw new Error(
      `Failed to fetch earnings data: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
...
export async function initiateTherapistPayout(request: PayoutRequest): Promise<{
  ...
  try {
    ...
    throw new Error(
      `Failed to initiate payout: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
