...
class HubSpotService {
  ...
  async createContact(contact: HubSpotContact): Promise<{ success: boolean; contactId?: string; error?: string }> {
    try {
      const properties = {
        email: contact.email,
  ...
  async updateContact(contactId: string, properties: Partial<HubSpotContact>): Promise<{ success: boolean; error?: string }> {
    try {
      const data = { properties };
  ...
  async findContactByEmail(email: string): Promise<{ success: boolean; contact?: any; error?: string }> {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Contact not found' };
    }
  }

  async createDeal(deal: HubSpotDeal): Promise<{ success: boolean; dealId?: string; error?: string }> {
    try {
      const properties = {
        ...
        closedate: deal.closedate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] // 30 days from now
      };
      ...
      if (deal.associatedVids && deal.associatedVids.length > 0) {
        for (const contactId of deal.associatedVids) {
          await this.makeRequest(`/crm/v3/objects/deals/${result.id}/associations/contacts/${contactId}/deal_to_contact`, 'PUT');
        }
      }
