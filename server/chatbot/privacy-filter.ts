...
const HEALTH_SENSITIVE_TERMS = [
  'medication', 'prescription', 'diagnosis', 'medical record', 'hospital',
  'GP', 'doctor', 'psychiatrist', 'therapist name', 'clinic',
  'medical history', 'treatment plan', 'medication name'
];
...
export function containsHealthInformation(text: string): boolean {
  const lowerText = text.toLowerCase();
  return HEALTH_SENSITIVE_TERMS.some(term => 
