import {
  appointmentMigrationService,
  createMigrationService,
} from "./appointment-migration-service";
import { db } from "./db";
...
export class AppointmentMigrationTest {
  ...
  async testDryRunMigration(): Promise<{ success: boolean; results: any; errors?: string[] }> {
    try {
      ...
      console.log(
        `📊 Results: ${summary.successful} successful, ${summary.failed} failed, ${summary.skipped} skipped`
      );
   ...
   */
  async testMigrationVerification(): Promise<{
    success: boolean;
    results: any;
    errors?: string[];
  }> {
    try {
      ...
      const sampleAppointments = await db
        .select()
        .from(appointments)
        .where(and(ne(appointments.status, "cancelled"), gte(appointments.scheduledAt, new Date())))
        .limit(1);
      ...
      console.log(
        `✅ Verification test completed: ${verificationResult.success ? "PASS" : "FAIL"}`
      );
   ...
   */
  async testMigrationServiceConfiguration(): Promise<{
    success: boolean;
    results: any;
    errors?: string[];
  }> {
