...
export class PaymentConfirmationService {
  ...
  static async confirmPayment(
    ...
    console.log(`🔄 PAYMENT CONFIRMATION: Starting for session ${sessionId} with payment ${paymentIntentId}`);
    ...
    while (attempts <= retryOptions.maxRetries) {
      ...
      try {
        console.log(`🔄 Payment confirmation attempt ${attempts}/${retryOptions.maxRetries + 1} for ${paymentIntentId}`);
        ...
        if (paymentIntent.status === 'succeeded') {
          ...
          console.log(`⚠️ Payment ${paymentIntentId} requires client action: ${paymentIntent.status}`);
          
          await this.recordPaymentAttempt(paymentRecord.id, attempts, 'requires_action', paymentIntent.status);
          ...
          return {
            ...
            requiresAction: {
              type: paymentIntent.status === 'requires_payment_method' ? 'payment_method_required' : 'authentication_required',
          ...
          await this.recordPaymentAttempt(paymentRecord.id, attempts, 'processing', 'Payment still processing');
          ...
          await this.recordPaymentFailure(paymentRecord, paymentIntent.status, paymentIntent.last_payment_error?.message);
          ...
          return {
            ...
            lastError: paymentIntent.last_payment_error?.message || `Payment ${paymentIntent.status}`
          };
          
        } else {
          // Unexpected status - log and retry
          lastError = `Unexpected payment status: ${paymentIntent.status}`;
          console.log(`❓ Unexpected payment status ${paymentIntent.status} for ${paymentIntentId}`);
          
          await this.recordPaymentAttempt(paymentRecord.id, attempts, 'unexpected_status', paymentIntent.status);
        }
        
      } catch (stripeError) {
        lastError = stripeError instanceof Error ? stripeError.message : 'Unknown Stripe error';
        console.error(`❌ Stripe error during payment confirmation attempt ${attempts}:`, stripeError);
    ...
    console.log(`❌ Payment confirmation failed after ${attempts} attempts for ${paymentIntentId}: ${lastError}`);
    
    await this.recordPaymentFailure(paymentRecord, 'failed', lastError || 'Maximum retries exhausted');
  ...
  private static async completePaymentConfirmation(
    ...
    try {
      ...
      await this.recordPaymentAttempt(paymentRecord.id, attempts, 'succeeded', 'Payment confirmed successfully');
      ...
      console.log(`💰 ELEMENT #5: Checking payout status before triggering for payment ${paymentRecord.id} on session ${sessionId}`);
      ...
      if (paymentRecord.payoutCompleted) {
        console.log(`🛡️ [RACE PROTECTION] Payout already completed for payment ${paymentRecord.id} - skipping duplicate trigger`);
      } else {
        ...
        try {
          ...
          if (paymentRecord.payoutTransferId || paymentRecord.payoutMethod === 'post_transfer') {
            console.log(`🛡️ [MULTI-TRIGGER PROTECTION] Payment ${paymentRecord.id} already has payout transfer ${paymentRecord.payoutTransferId} - skipping duplicate trigger`);
          } else {
        console.log(`💰 ELEMENT #5: Triggering therapist payout for confirmed payment ${paymentRecord.id} on session ${sessionId}`);
        ...
        try {
          ...
          if (payoutResult.success) {
            console.log(`✅ ELEMENT #5: Therapist payout ${payoutResult.payoutId} triggered successfully for payment ${paymentRecord.id}`);
          } else {
            console.error(`❌ ELEMENT #5: Therapist payout failed for payment ${paymentRecord.id}: ${payoutResult.message}`);
          ...
          console.error(`❌ ELEMENT #5: Unexpected error triggering therapist payout for payment ${paymentRecord.id}:`, payoutError);
          // Log but don't fail payment confirmation - payout can be retried
        }
          }
        } catch (protectionCheckError) {
          console.error(`❌ [MULTI-TRIGGER PROTECTION] Error checking existing payouts:`, protectionCheckError);
          // Continue with payout trigger on error - better to risk duplicate than miss payment
          console.log(`💰 ELEMENT #5: Triggering therapist payout for confirmed payment ${paymentRecord.id} on session ${sessionId} (protection check failed)`);
      ...
      return {
        ...
        lastError: completionError instanceof Error ? completionError.message : 'Unknown completion error'
      };
    }
  }
   ...
   */
  private static async recordPaymentFailure(paymentRecord: any, status: string, error?: string): Promise<void> {
    try {
      // Update payment record
      await storage.updatePaymentByStripeId(paymentRecord.stripePaymentIntentId, {
        status: 'failed',
  ...
  private static async recordPaymentAttempt(
    ...
    try {
      ...
      console.log(`📋 PAYMENT AUDIT: Payment ${paymentId} attempt ${attemptNumber}: ${status}${details ? ` - ${details}` : ''}`);
