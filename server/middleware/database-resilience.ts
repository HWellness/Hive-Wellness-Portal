  ...
  async checkConnection(): Promise<boolean> {
    try {
      ...
      if (error.message?.includes('WebSocket') || error.message?.includes('ws')) {
        console.error('🚨 WebSocket connection issue detected - this may be a Railway environment compatibility issue');
        console.log('💡 Suggestion: Check if Railway supports WebSocket connections for Neon serverless');
      }
      
      if (error.message?.includes('timeout') || error.code === 'ETIMEDOUT') {
        console.error('⏱️ Database connection timeout - Railway may have network connectivity issues');
      }
...
export const databaseResilienceMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
