  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (Math.random() < 0.1) {
      for (const [k, v] of Array.from(rateLimitStore.entries())) {
        if (v.resetTime < now && v.lastViolation < now - ENHANCED_RATE_LIMIT_CONFIG.VIOLATION_RESET_TIME) {
    ...
    if (current.count >= burstLimit) {
      ...
      logSecurityEvent({
        type: 'rate_limit',
        severity: current.violations >= ENHANCED_RATE_LIMIT_CONFIG.MAX_VIOLATIONS_BEFORE_BLOCK ? 'high' : 'medium',
        ip: key,
        userAgent: req.get('User-Agent') || 'unknown',
        userId: user?.id,
        details: {
