...
export const securityHeaders = (req: Request, res: Response, next: NextFunction) => {
  ...
  const isIframeRoute = req.path === '/book-admin-call-client-new' || 
  ...
  if (isIframeRoute) {
    ...
    res.setHeader('Permissions-Policy', 'geolocation=(), microphone=(), camera=(), fullscreen=(self), gyroscope=(), payment=()');
  ...
  const allowIframeEmbedding = (req.path === '/iframe/portal' || 
      req.path === '/portal' ||
  ...
  console.log(`🔐 Path: ${req.path}, RefererCheck: ${req.headers.referer?.includes('hive-wellness.co.uk')}, AllowIframe: ${allowIframeEmbedding}`);
  ...
  if (allowIframeEmbedding) {
    console.log(`🔐 Iframe embedding allowed for path: ${req.path}, referer: ${req.headers.referer}`);
  }
...
export const csrfProtection = (req: Request, res: Response, next: NextFunction) => {
  ...
  const webhookPaths = ['/api/stripe/webhook', '/api/hubspot/webhook', '/api/gravity-forms/webhook'];
  if (webhookPaths.some(path => req.path === path)) {
    return next();
  }
  
  // Skip for public booking endpoints (no session required)
  const publicPaths = ['/api/auth/login', '/api/auth/register', '/api/book-widget', '/api/book-introduction-call'];
