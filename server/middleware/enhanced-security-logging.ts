...
export const enhancedFormValidation = (req: Request, res: Response, next: NextFunction) => {
  const isProfileEndpoint = ['/api/client/profile', '/api/therapist/profile'].some(
    endpoint => req.path.includes(endpoint)
  );
  ...
  logSecurityEvent({
    ...
    method: req.method,
    details: {
      reason: validationResults.isValid ? 'Profile data validation passed' : 'Profile data validation failed',
