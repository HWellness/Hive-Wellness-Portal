...
export const logEnhancedAuditEvent = (event: Omit<EnhancedAuditEvent, 'id' | 'timestamp' | 'riskScore'>, isSystemGenerated: boolean = false) => {
...
const calculateRiskScore = (event: Omit<EnhancedAuditEvent, 'id' | 'timestamp' | 'riskScore'>): number => {
  let score = 0;
...
const detectSuspiciousPatterns = (event: EnhancedAuditEvent, ipKey: string) => {
  ...
  if (failedEvents.length >= 10) {
    logEnhancedAuditEvent({
      ...
      method: 'SYSTEM'
    }, true); // Mark as system-generated to prevent recursion
  }
...
export const comprehensiveAuditMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.send = function(data: any) {
    ...
    if (req.url.includes('/api/auth/') || req.url.includes('/login') || req.url.includes('/logout')) {
      category = 'security';
...
export const getEnhancedAuditStats = (hours: number = 24) => {
  ...
  return {
    ...
    averageRiskScore: allEvents.length > 0 ? allEvents.reduce((sum, e) => sum + e.riskScore, 0) / allEvents.length : 0,
    topRiskIPs: getTopRiskIPs(allEvents),
    complianceEvents: allEvents.filter(e => e.complianceFlags && e.complianceFlags.length > 0).length,
    sensitiveDataAccess: allEvents.filter(e => e.dataAccessed && e.dataAccessed.length > 0).length
  };
};
