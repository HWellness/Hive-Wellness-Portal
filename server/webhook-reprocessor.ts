  ...
  async reprocessStuckWebhooks(): Promise<void> {
    ...
    for (const webhookInfo of STUCK_WEBHOOK_EVENTS) {
      try {
        ...
        const result = await this.webhookProcessor.processStripeWebhook(
          eventData, 
          reprocessingId
        );
    ...
    if (successful.length > 0) {
      ...
      successful.forEach(result => {
        console.log(`  - ${result.webhookId}: ${result.alreadyProcessed ? 'Already processed' : 'Newly processed'}`);
  ...
  async verifyCompletionStatus(): Promise<void> {
    ...
    for (const webhookInfo of STUCK_WEBHOOK_EVENTS) {
      try {
        ...
        console.log(`📋 ${webhookInfo.dbId}: ${webhookEvent.processingStatus}${
