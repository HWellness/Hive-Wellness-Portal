 ...
 */
export async function createSessionPaymentWithHold(sessionData: {
  sessionFee: number;
  therapistStripeAccountId: string;
  sessionId: string;
  scheduledDate: string;
 ...
 */
export async function releaseSessionPayment(paymentIntentId: string): Promise<{
  transferId: string;
  amount: number;
  ...
  try {
    ...
    throw new Error(
      `Failed to release payment: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
...
export async function getPaymentsReadyForRelease(): Promise<SessionPaymentHold[]> {
  try {
    ...
    for (const pi of paymentIntents.data) {
      if (
        pi.metadata.paymentHoldStatus === "held" &&
    ...
    throw new Error(
      `Failed to get pending payments: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
  ...
  try {
    ...
    for (const payment of readyPayments) {
      try {
        ...
        console.log(
          `✅ Released payment ${payment.paymentIntentId} for session ${payment.sessionId}`
        );
      } catch (error) {
        failed++;
        results.push({
