...
export class ReminderAutomationService {
          ...
          try {
            ...
            if (reminderTime > now && reminderTime <= twoDaysFromNow) {
              ...
              const client = appointment.clientId
                ? await this.storage.getUser(appointment.clientId)
                : null;
              ...
              if (config.recipientRole === "client" || config.recipientRole === "both") {
                ...
                if (!existingClientReminder) {
                  ...
                  created++;
                  console.log(
                    `✅ Created client reminder for appointment ${appointment.id} (${config.timeBefore} min before)`
                  );
                }
              }
              ...
              if (config.recipientRole === "therapist" || config.recipientRole === "both") {
                const existingTherapistReminder =
                  await this.storage.getReminderByAppointmentAndConfig(
                    appointment.id,
                    config.id,
                    therapist.id
                  );
                ...
                if (!existingTherapistReminder) {
                  ...
                  created++;
                  console.log(
                    `✅ Created therapist reminder for appointment ${appointment.id} (${config.timeBefore} min before)`
                  );
                }
              }
            }
  ...
  private async createReminderQueueItem(
    ...
    message = message.replace(
      /\{\{client_name\}\}/g,
      recipient.role === "client" ? recipient.fullName || "Client" : otherParty.fullName || "Client"
    );
    message = message.replace(
      /\{\{therapist_name\}\}/g,
      recipient.role === "therapist"
        ? recipient.fullName || "Therapist"
        : otherParty.fullName || "Therapist"
    );
    message = message.replace(
      /\{\{session_date\}\}/g,
    ...
    await this.storage.createReminderQueueItem({
      ...
      subject:
        config.subject ||
        `Appointment Reminder - ${new Date(appointment.scheduledAt).toLocaleDateString("en-GB")}`,
      message: message,
