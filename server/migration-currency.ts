...
export async function migrateCurrencyData() {
  ...
  try {
    ...
    const existingAccounts = await db.select().from(workspaceAccounts)
    ...
    for (const account of existingAccounts) {
      try {
        ...
        const gbpAmount = planCosts[account.planType as keyof typeof planCosts] || usdAmount * 0.80;
        
        await db.update(workspaceAccounts)
          .set({
            currency: 'GBP',
    ...
    console.log(`✅ Step 1 Complete: Migrated ${migratedAccounts}/${existingAccounts.length} workspace accounts`);
    ...
    for (const report of existingReports) {
      try {
        ...
        const gbpAmount = usdAmount * 0.80; // USD to GBP conversion rate
        
        await db.update(costReports)
          .set({
        ...
        migratedReports++;
        console.log(`✅ Migrated cost report ${report.month}: $${usdAmount} USD → £${gbpAmount} GBP`);
      } catch (error) {
        console.error(`❌ Failed to migrate cost report ${report.id}:`, error);
      }
    }
    
    console.log(`✅ Step 2 Complete: Migrated ${migratedReports}/${existingReports.length} cost reports`);
    ...
    for (const budget of existingBudgets) {
      try {
        ...
        migratedBudgets++;
        console.log(`✅ Migrated budget ${budget.budgetName}: $${usdAmount} USD → £${gbpAmount} GBP`);
      } catch (error) {
        console.error(`❌ Failed to migrate budget ${budget.id}:`, error);
      }
    }
    
    console.log(`✅ Step 3 Complete: Migrated ${migratedBudgets}/${existingBudgets.length} cost budgets`);
    
    // Step 4: Verification
    console.log('🔍 Step 4: Verifying migration results...');
    
    const verificationAccounts = await db.select().from(workspaceAccounts)
    ...
    throw new Error(`Currency migration failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
