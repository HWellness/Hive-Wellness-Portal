...
export async function createPaymentWithRevenueSplit(
  ...
  if (!accountStatus.chargesEnabled) {
    throw new Error(`Therapist account not ready for payments. Account status: ${accountStatus.status}. Charges enabled: ${accountStatus.chargesEnabled}`);
  }
  
  if (!accountStatus.payoutsEnabled) {
    throw new Error(`Therapist account cannot receive payouts. Account status: ${accountStatus.status}. Payouts enabled: ${accountStatus.payoutsEnabled}`);
  }
 ...
 */
export async function createTherapistStripeAccount(
  therapistData: {
    email: string;
    firstName: string;
    lastName: string;
  ...
  try {
    ...
    throw new Error(`Failed to create therapist Stripe account: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function processTherapistPayout(
  ...
  try {
    ...
    if (idempotencyKey) {
      ...
      console.log(`🛡️ [FINANCIAL SAFETY] Creating Stripe transfer with idempotency key: ${idempotencyKey}`);
    } else {
      console.warn(`⚠️ [FINANCIAL SAFETY] Creating Stripe transfer without idempotency key - not recommended for production`);
    }
...
export async function processInstantTherapistPayout(
  ...
  try {
    ...
    console.log(`💰 [INSTANT PAYOUT] Processing instant payout: £${amount} (fee: £${fee}, net: £${netAmount})`);
    ...
    if (idempotencyKey) {
      ...
      console.log(`🛡️ [FINANCIAL SAFETY] Creating instant payout with idempotency key: ${idempotencyKey}`);
    }
    ...
    console.log(`💵 [INSTANT PAYOUT] Amount: £${netAmount}, Fee: £${fee}, Status: ${payout.status}`);
