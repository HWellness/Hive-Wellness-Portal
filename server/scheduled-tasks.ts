...
export class ScheduledTasksService {
  ...
  private scheduleHubSpotImport() {
    ...
    cron.schedule('0 * * * *', async () => {
      ...
      try {
        ...
        if (results.clientQuestionnaires.imported > 0 || results.therapistApplications.imported > 0) {
  ...
  private scheduleGravityFormsSync() {
    cron.schedule('*/30 * * * *', async () => {
      ...
      try {
        ...
        console.log(`✅ Scheduled Gravity Forms sync completed: ${totalProcessed} entries processed`);
  ...
  private scheduleAppointmentReminders() {
    ...
    cron.schedule('30 * * * *', async () => {
      ...
      try {
        const { ReminderAutomationService } = await import('./services/reminder-automation-service.js');
        ...
        console.log(`✅ Reminder queue creation completed: ${createResult.created} created, ${createResult.errors} errors`);
  ...
  private scheduleReminderProcessing() {
    ...
    cron.schedule('*/15 * * * *', async () => {
      ...
      try {
        ...
        console.log(`✅ Reminder processing completed: ${sendResult.sent} sent, ${sendResult.failed} failed`);
  ...
  private scheduleRetentionWarnings() {
    cron.schedule('0 10 * * *', async () => {
      ...
      try {
          ...
          try {
            ...
            const emailBody = `
              <div style="font-family: 'Open Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <p>This is a notice that your Hive Wellness account is scheduled for deletion on <strong>${deletionDate.toLocaleDateString('en-GB', { 
                  day: 'numeric', 
                  month: 'long', 
                  year: 'numeric' 
        ...
        console.log(`✅ Deletion warning emails completed: ${emailsSent} sent, ${emailsFailed} failed`);
    ...
    return {
      ...
      tasks: {
        ...
        dataCleanup: {
          ...
          description: 'HIPAA data retention, GDPR export cleanup (>7 days), and deletion request processing'
        },
        reminderQueueCreation: {
          schedule: 'Every hour at minute 30',
          description: 'Create reminder queue items from appointments based on admin configurations'
        },
        reminderProcessing: {
