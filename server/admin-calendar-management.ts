...
export class AdminCalendarManager {
  ...
  async checkTimeSlotAvailability(date: Date, timeSlot: string): Promise<TimeSlotCheck> {
    ...
    console.log(`🔍 Checking Google Calendar availability for ${timeSlot} on ${date.toDateString()}`);
    const googleCalendarAvailable = await calendarBookingSync.isTimeSlotAvailable(slotStart, bufferEnd);
    
    if (!googleCalendarAvailable) {
      console.log(`🚫 Google Calendar blocks this time slot: ${timeSlot} on ${date.toDateString()}`);
      return {
        date,
        timeSlot,
        isAvailable: false,
    ...
    if (existingCalls.length > 0 || conflictingCalls.length > 0) {
      return {
        ...
        timeSlot,
        isAvailable: false,
        conflictReason: existingCalls.length > 0 
