...
const templates: TemplateConfig[] = [
  ...
  // WhatsApp TEMPLATES - Booking Confirmations  
  {
    ...
    placeholders: ['client_name', 'therapist_name', 'appointment_date', 'appointment_time', 'meeting_link']
  },
  {
  ...
  try {
    ...
    for (const template of templates) {
      ...
      const exists = existingTemplates.some(t => 
        t.name === template.name && t.channel === template.channel
      );

      if (exists) {
        console.log(`⏭️  Template "${template.name}" (${template.channel}) already exists, skipping...`);
        skippedCount++;
        continue;
      }
