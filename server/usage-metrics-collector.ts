...
export class UsageMetricsCollector {
   ...
   */
  async collectTherapistUsage(therapistId: string, month?: string): Promise<UsageCollectionResult> {
  ...
  private async collectComprehensiveMetrics(
    ...
    if (this.initialized) {
      try {
        ...
        const driveMetrics = await this.getDriveMetrics(
          workspaceAccount.workspaceEmail,
          startDate,
          endDate
        );
        ...
        console.warn(
          `⚠️ Some Google API metrics unavailable for ${workspaceAccount.workspaceEmail}:`,
          error
        );
      }
    }
  ...
  private async getStorageUsage(workspaceEmail: string): Promise<number> {
    ...
    try {
      ...
      if (usageReports.length > 0) {
        const usedQuotaMB =
          usageReports[0].parameters?.find((p: any) => p.name === "accounts:used_quota_in_mb")
            ?.intValue || 0;
  ...
  private async getDriveMetrics(
    ...
    try {
      ...
      const totalSizeBytes = files.reduce(
        (sum: number, file: any) => sum + parseInt(file.size || "0", 10),
        0
      );
    ...
    for (let i = 0; i < activeAccounts.length; i += batchSize) {
      ...
      batchResults.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          console.error(
            `❌ Failed to collect usage for therapist ${batch[index].therapistId}:`,
            result.reason
          );
        }
      });
  ...
  private calculateSystemSummary(
    ...
    const totalCalendarEvents = results.reduce(
      (sum, r) => sum + r.metrics.calendarEventsCreated,
      0
    );
    ...
    const collectionCompleteness =
      totalTherapists > 0 ? (successfulCollections / totalTherapists) * 100 : 100;
