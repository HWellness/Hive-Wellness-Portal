...
export class GoogleCalendarService {
    ...
    try {
      if (process.env.GOOGLE_SERVICE_ACCOUNT_KEY) {
        ...
        this.testCalendarAccess().catch((error) => {
          console.warn(
            "⚠️ Calendar access test failed but service is ready:",
            error?.message || error
          );
        });
      } else {
  ...
  private async ensureReady(): Promise<void> {
    ...
    if (!this.readyPromise) {
      ...
      this.readyPromise = Promise.race([initPromise, timeoutPromise]).catch((error) => {
  ...
  async getTherapistCalendarId(therapistId: string): Promise<string> {
    ...
    try {
      ...
      if (
        therapistCalendarInfo.isConfigured &&
        therapistCalendarInfo.calendarId !== this.adminCalendarId
      ) {
      ...
      console.log(
        `⚠️ Therapist ${therapistId} calendar not configured, using admin calendar fallback`
      );
      return this.adminCalendarId;
    } catch (error: any) {
      console.error(
        `❌ Error resolving calendar for therapist ${therapistId}:`,
        error?.message || error
      );
  ...
  async getTherapistCalendarInfo(therapistId: string): Promise<TherapistCalendarInfo> {
    ...
    try {
      ...
      console.error(
        `❌ Error getting calendar info for therapist ${therapistId}:`,
        error?.message || error
      );
  ...
  async validateTherapistCalendarAccess(therapistId: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `❌ Calendar access validation failed for therapist ${therapistId}:`,
        error?.message || error
      );
      return false;
    }
  ...
  async getTherapistBusyTimes(
    ...
    try {
      console.log(
        `🔍 Getting busy times for therapist ${therapistId}: ${timeRange.startTime.toISOString()} - ${timeRange.endTime.toISOString()}`
      );
      ...
      console.log(
        `📅 Found ${busyTimes.length} busy times for therapist ${therapistId}:`,
        busyTimes.map((bt) => ({
      ...
      console.error(
        `❌ Error getting busy times for therapist ${therapistId}:`,
        error?.message || error
      );
  ...
  async createBlockingEvent(eventData: {
    ...
    try {
      ...
      console.log(
        `✅ Created blocking event: ${eventData.title} on calendar ${targetCalendarId} (${eventData.startTime.toISOString()} - ${eventData.endTime.toISOString()})`
      );
      return true;
    } catch (error) {
  ...
  async createAvailabilityBlockingEvents(
    dayOfWeek: number,
    startTime: string,
    endTime: string,
    ...
    try {
      ...
      if (therapistId) {
        ...
        console.log(
          `📅 Creating availability blocks for therapist ${therapistId} on calendar ${targetCalendarId}`
        );
      } else {
        targetCalendarId = this.adminCalendarId;
      ...
      console.log(
        `✅ Created ${createdEvents} blocking events for ${dayName} availability (${startTime}-${endTime})`
      );
  ...
  async updateEvent(
    ...
    try {
      ...
      console.log(
        `✅ Updated event ${eventId} on calendar ${targetCalendarId}: ${eventData.title}`
      );
      return true;
    } catch (error) {
  ...
  async deleteBlockingEvents(
    ...
    try {
      ...
      console.log(
        `✅ Deleted ${deletedCount} blocking events matching pattern: ${titlePattern} from calendar ${targetCalendarId}`
      );
      return deletedCount;
    } catch (error) {
   ...
   */
  private determineCalendarRouting(
    sessionType?: string,
    therapistId?: string,
    useAdminCalendar?: boolean
  ): {
    ...
    return await this.safeCalendarOperation(
      async () => {
        ...
        if (routing.useAdmin) {
          ...
          console.log(
            `📅 THERAPIST CALENDAR: Resolved therapist ${sessionData.therapistId} calendar to: ${targetCalendarId}`
          );
...
Need help? Contact support@hive-wellness.co.uk
        ...
        const londonStartTime = formatInTimeZone(
          sessionData.startTime,
          "Europe/London",
          "yyyy-MM-dd'T'HH:mm:ss"
        );
        const londonEndTime = formatInTimeZone(
          sessionData.endTime,
          "Europe/London",
          "yyyy-MM-dd'T'HH:mm:ss"
        );
        ...
        console.log(
          `🌍 Converting UTC to London time - Start: ${sessionData.startTime.toISOString()} → ${londonStartTime}, End: ${sessionData.endTime.toISOString()} → ${londonEndTime}`
        );
        ...
        console.log(
          `📅 Time: ${sessionData.startTime.toISOString()} - ${sessionData.endTime.toISOString()}`
        );
        ...
        console.log(
          `✅ Session calendar event created successfully: ${eventId} on calendar ${targetCalendarId}`
        );
        console.log(
          `📅 Event URL: https://calendar.google.com/calendar/event?eid=${response.data.id}`
        );
  ...
  async updateSessionEvent(
    ...
    try {
      ...
      console.log(
        `✅ Session calendar event updated successfully: ${eventId} on calendar ${targetCalendarId}`
      );
      return true;
    } catch (error: any) {
  ...
  async deleteSessionEvent(
    ...
    try {
      ...
      console.log(
        `📅 Deleting session calendar event: ${eventId} from calendar ${targetCalendarId}`
      );
      ...
      console.log(
        `✅ Session calendar event deleted successfully: ${eventId} from calendar ${targetCalendarId}`
      );
      return true;
    } catch (error: any) {
      if (error.code === 404) {
  ...
  async testSessionEventCreation(): Promise<{
    ...
    try {
      ...
      const testSessionData = {
        ...
        description:
          "This is a test session to verify enhanced Google Calendar integration with attendees and Google Meet.",
      ...
      console.log(
        `🧪 Attendees: ${testSessionData.attendees.map((a) => `${a.name} (${a.email})`).join(", ")}`
      );
  ...
  async backfillCalendarEvents(): Promise<{
    ...
    failed: Array<{
      appointmentId: string;
      error: string;
      type: "appointment" | "introduction_call";
    }>;
    summary: string;
  }> {
    try {
      ...
      for (const appointment of appointmentsToProcess) {
        processed++;
        console.log(
          `📅 Processing appointment ${processed}/${appointmentsToProcess.length}: ${appointment.id}`
        );
        ...
        try {
          ...
          console.error(
            `❌ Failed to create calendar event for appointment ${appointment.id}:`,
            error.message
          );
          failed.push({
            appointmentId: appointment.id,
