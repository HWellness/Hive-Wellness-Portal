  ...
  import('./db').then(async ({ db }) => {
    try {
      ...
      if (constraintCheck.rowCount === 0) {
        ...
        console.log('✅ Database constraint validation: appointments_no_overlap constraint is active');
        ...
        if (extensionCheck.rowCount === 0) {
          ...
          console.error('🚨 The appointments_no_overlap constraint requires btree_gist extension');
        } else {
...
try {
  ...
  import('./google-calendar-service').then(async ({ googleCalendarService }) => {
    ...
    if (googleCalendarService) {
      ...
      try {
        const canAccess = await googleCalendarService.validateTherapistCalendarAccess('test-startup');
        console.log('🔐 Calendar authentication test result:', canAccess ? '✅ SUCCESS' : '❌ FAILED');
      } catch (authError) {
    ...
    console.error('🔧 Check Google service account configuration and ensure all dependencies are installed');
  });
} catch (error) {
...
(async () => {
  try {
    ...
    try {
      ...
      console.error('❌ Database schema health check failed:', error instanceof Error ? error.message : 'Unknown error');
