...
export class VideoBookingService {
  ...
  static async bookVideoSession(request: BookingRequest): Promise<VideoSessionBooking> {
    try {
      ...
      console.log(`🔍 Checking time slot conflict BEFORE creating calendar event: ${sessionDateTime.toISOString()} - ${sessionEndTime.toISOString()}`);
      const existingBooking = await this.checkTimeSlotConflict(sessionDateTime, sessionEndTime, request.therapistId);
      if (existingBooking) {
        console.log(`❌ Time slot conflict detected:`, existingBooking);
        throw new Error(`Time slot ${request.date} ${request.time} is already booked. Existing booking: ${existingBooking.clientName}`);
      }
      ...
      try {
        ...
        if (routing.useAdminCalendar) {
          ...
          console.log(`📅 ADMIN ROUTING: Creating ${request.sessionType} in admin calendar (support@hive-wellness.co.uk)`);
          ...
          console.log(`📅 THERAPIST ROUTING: Creating ${request.sessionType} with Daily.co + therapist ${request.therapistId} calendar`);
...
ROUTED TO: Therapist Calendar (${routing.reason})`;
            ...
            calendarEvent = await googleCalendarService.createSessionEvent({
              ...
              attendees: [
                ...
                { name: 'Hive Wellness Support', email: 'support@hive-wellness.co.uk', role: 'therapist' as const }
              ],
              therapistId: request.therapistId,
              sessionType: request.sessionType
            });
            
            console.log(`✅ Created ${request.sessionType} in therapist ${request.therapistId} calendar successfully`);
          } catch (therapistCalendarError) {
            console.warn(`⚠️ Failed to create event in therapist calendar, continuing with Daily.co room:`, therapistCalendarError);
        ...
        if (calendarEvent) {
          if (calendarEvent && typeof calendarEvent === 'object' && 'eventId' in calendarEvent) {
            ...
            const sessionEvent = calendarEvent as { eventId: string; meetingUrl: string; meetingId: string };
            ...
            booking.calendarEventId = typeof calendarEvent === 'string' ? calendarEvent : (calendarEvent as any).eventId || '';
          }
        }
      ...
      try {
        ...
        if (appointmentError.code === 'APPOINTMENT_OVERLAP') {
          console.error('❌ Database constraint violation - appointment overlap detected:', appointmentError.details);
          throw {
      ...
      throw new Error(`Failed to book video session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  static async checkTimeSlotConflict(startTime: Date, endTime: Date, therapistId?: string): Promise<any | null> {
    try {
      console.log(`🔍 COMPREHENSIVE conflict check for: ${startTime.toISOString()} - ${endTime.toISOString()}${therapistId ? ` (Therapist: ${therapistId})` : ''}`);
      ...
      if (therapistId) {
        try {
          ...
          if (hasOverlap) {
            ...
            console.log(`❌ THERAPIST CALENDAR CONFLICT detected - therapist ${therapistId} is busy:`, {
          ...
          const dbConflicts = await storage.getOverlappingAppointments(startTime, endTime, therapistId);
          
          if (dbConflicts.length > 0) {
            console.log(`❌ DATABASE CONFLICT detected - therapist ${therapistId} has overlapping appointments:`, 
          ...
          console.log(`✅ Database check passed - no conflicting appointments for therapist ${therapistId}`);
          
        } catch (therapistCalendarError) {
          console.error(`⚠️ Error checking therapist ${therapistId} calendar, falling back to admin calendar:`, therapistCalendarError);
      ...
      try {
        ...
        if (!isTimeAvailable) {
          console.log(`❌ GOOGLE CALENDAR CONFLICT detected - time slot blocked by calendar events`);
          return {
        ...
        console.error('⚠️ Google Calendar check failed, continuing with database checks:', calendarError);
      }
      ...
      const blockedSlots = conflictingBlocks.filter(block => 
      ...
      if (blockedSlots.length > 0) {
        console.log(`❌ Found ${blockedSlots.length} conflicting blocked slots:`, blockedSlots.map(slot => ({
          title: slot.title,
      ...
      if (appointments.length > 0) {
        console.log(`❌ Found ${appointments.length} conflicting appointments:`, appointments.map(appt => ({
          clientName: appt.clientName,
    ...
    try {
      ...
      if (booking.calendarEventId && booking.therapistId) {
        try {
          ...
          console.log(`📅 Deleted calendar event ${booking.calendarEventId} from therapist ${booking.therapistId} calendar`);
        } catch (error) {
          ...
          try {
            ...
            console.error('❌ Failed to delete calendar event from both therapist and admin calendars:', fallbackError);
          }
        }
      }
      ...
      throw new Error(`Failed to cancel video session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static async getAvailableSlots(therapistId: string, date: string): Promise<string[]> {
    try {
      ...
      try {
        ...
        const lunchStart = adminSettings.includeLunchBreak ? new Date(`2024-01-01T${adminSettings.lunchBreakStart}:00`) : null;
        const lunchEnd = adminSettings.includeLunchBreak ? new Date(`2024-01-01T${adminSettings.lunchBreakEnd}:00`) : null;
        
        let currentSlot = new Date(startTime);
        
        while (currentSlot < endTime && defaultSlots.length < (adminSettings.maxSessionsPerDay || 8)) {
          ...
          if (lunchStart && lunchEnd && currentSlot >= lunchStart && currentSlot < lunchEnd) {
            currentSlot = new Date(currentSlot.getTime() + (adminSettings.sessionDuration * 60 * 1000));
            continue;
          }
          ...
          const totalSlotTime = adminSettings.sessionDuration + (adminSettings.bufferTimeBetweenSessions || 0);
        ...
        return defaultSlots.length > 0 ? defaultSlots : ['09:00', '09:30', '10:00', '10:30', '11:00', '11:30'];
      } catch (settingsError) {
