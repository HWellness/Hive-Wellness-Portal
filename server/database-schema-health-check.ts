...
const CRITICAL_SCHEMA_REQUIREMENTS = {
  ...
  therapist_profiles: [
    ...
    "workspace_last_login",
    "calendar_permissions_configured",
    "workspace_account_notes",
    "created_at",
    "updated_at",
  ],
  ...
  refunds: [
    ...
    "original_amount",
    "refund_amount",
    "therapist_compensation",
    "stripe_processing_fee_retained",
    "refund_percentage",
    "refund_reason",
    "cancellation_time",
    "session_time",
...
const REQUIRED_TABLES = [
  ...
  "wordpress_booking_integration",
  "document_retention_policies",
  "institution_onboarding",
  "stripe_connect_applications",
  "cost_optimizations",
  "sessions",
  "user_sessions",
  "therapy_categories",
  ...
  "form_submissions",
  "automated_workflows",
  "therapist_matching_questionnaires",
  "therapist_enquiries",
...
export class DatabaseSchemaHealthCheck {
  ...
  static async performHealthCheck(): Promise<SchemaHealthCheckResult> {
    ...
    try {
      ...
      result.errors.push(
        `Schema health check failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  ...
  private static async checkRequiredTables(result: SchemaHealthCheckResult): Promise<void> {
    try {
      ...
      result.errors.push(
        `Failed to check tables: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
    ...
    for (const [tableName, requiredColumns] of Object.entries(CRITICAL_SCHEMA_REQUIREMENTS)) {
      try {
        ...
        result.warnings.push(
          `Could not check columns for table ${tableName}: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      }
    }
  }
  ...
  private static generateSummary(result: SchemaHealthCheckResult): void {
    const totalIssues =
      result.errors.length + result.missingTables.length + result.missingColumns.length;
