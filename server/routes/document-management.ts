...
const DOCUMENT_LIBRARY = [
  ...
  },
  {
    ...
    description: 'Complete guide to working with Hive Wellness including values, expectations, session structure, tiered pricing system, and earnings breakdown.',
...
router.get('/documents/:documentId', isAuthenticated, async (req, res) => {
  try {
    ...
    // Check permissions
    if (userRole !== 'admin' && 
        (userRole === 'client' && document.category !== 'client') ||
        (userRole === 'therapist' && document.category !== 'therapist' && document.category !== 'client')) {
