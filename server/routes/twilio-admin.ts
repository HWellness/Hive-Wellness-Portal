...
router.get("/templates", requireAdmin, async (req, res) => {
  try {
    ...
    if (active !== undefined)
      whereConditions.push(eq(notificationTemplates.isActive, active === "true"));
...
router.post("/templates", requireAdmin, async (req, res) => {
  try {
    ...
    const existing = await db
      .select()
      .from(notificationTemplates)
      .where(and(eq(notificationTemplates.name, name), eq(notificationTemplates.channel, channel)));
    ...
    if ((channel === "sms" || channel === "whatsapp") && !body.toLowerCase().includes("stop")) {
      return res.status(400).json({
        success: false,
        error:
          'SMS and WhatsApp templates must include opt-out instructions (e.g., "Reply STOP to opt out")',
      });
    }
  ...
  try {
    ...
    if (usageCheck[0]?.count > 0) {
      return res.status(400).json({
        success: false,
        error:
          "Cannot delete template that has been used in notifications. Consider deactivating instead.",
      });
    }
