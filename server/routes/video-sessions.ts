  ...
  try {
    ...
    const isParticipant =
      ...
      (sessionData.sessionType === "introduction-call" &&
        userEmail &&
        sessionData.clientEmail === userEmail);

    if (!isParticipant) {
      console.log(
        `Access denied: User ${userId} (${userEmail}) is not a participant in session ${sessionId}`,
        {
          clientId: sessionData.clientId,
          therapistId: sessionData.therapistId,
          clientEmail: sessionData.clientEmail,
...
router.get("/introduction-call/:bookingId/access", async (req: any, res: any) => {
  try {
    ...
    if (introCall.email.toLowerCase() !== email.toLowerCase()) {
      console.log(
        `Email mismatch for booking ${bookingId}: expected ${introCall.email}, got ${email}`
      );
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      console.log(
        `User ${userId} attempting to join video session ${sessionId} (bypass payment: ${bypassPayment})`
      );
      ...
      if (result.success) {
        ...
        const response = {
          ...
          message: bypassPayment
            ? "Successfully joined confirmed session"
  ...
  async (req: any, res: any) => {
    try {
      ...
      if (!validationResult.success) {
        console.error(
          `❌ Invalid session completion request for ${sessionId}:`,
      ...
      const { idempotencyKey, actualDuration, paymentTiming, completionReason } =
      ...
      console.log(
        `🏁 PRODUCTION-READY session completion: ${sessionId} by ${userId} (${userRole})`
      );
      ...
      if (result.success) {
        ...
        const statusCode = result.error?.includes("ALREADY_COMPLETED")
          ? 400
          : result.error?.includes("PAYMENT_IN_PROGRESS")
            ? 409
            : result.error?.includes("NOT_AUTHORIZED")
              ? 403
              : 500;
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      if (!validationResult.success) {
        console.error(
          `❌ Invalid payment confirmation request for ${sessionId}:`,
          validationResult.error.flatten()
        );
        return res.status(400).json({
          success: false,
      ...
      const result = await SessionPaymentService.confirmSessionPayment(
        sessionId,
        paymentIntentId,
        userId
      );
      ...
      if (result.success) {
        ...
        const statusCode = result.error?.includes("NOT_FOUND")
          ? 404
          : result.error?.includes("ALREADY_")
            ? 400
            : 500;
      ...
      const { sessionId } = req.params;
      console.error(
        `❌ CRITICAL ERROR during payment confirmation for session ${sessionId}:`,
        error
      );
