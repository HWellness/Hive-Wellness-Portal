...
export function setupWorkspaceManagementRoutes(app: Express) {
  ...
  app.get('/api/therapists/:id/workspace-status', isAuthenticated, adminLimiter, async (req, res) => {
    try {
  ...
  app.post('/api/therapists/:id/provision-workspace', isAuthenticated, adminLimiter, ipSecurityCheck, async (req, res) => {
    try {
      const { id: therapistId } = req.params;
      ...
      const workflowResult = await therapistOnboardingService.processTherapistApproval(enquiry.id);
      ...
      if (workflowResult.workspaceProvisioned) {
        ...
        console.error(`❌ Manual workspace provisioning failed for ${user.email}: ${workflowResult.error}`);
  ...
  app.put('/api/therapists/:id/workspace-account', isAuthenticated, adminLimiter, ipSecurityCheck, async (req, res) => {
    try {
      const { id: therapistId } = req.params;
  ...
  app.post('/api/admin/bulk-provision-workspace', isAuthenticated, adminLimiter, ipSecurityCheck, async (req, res) => {
    try {
      ...
      console.log(`🎯 Bulk provisioning completed: ${successCount} successful, ${skippedCount} skipped, ${errors.length} errors`);
  ...
  app.get('/api/admin/workspace-audit', isAuthenticated, adminLimiter, async (req, res) => {
    try {
      ...
      const auditData = await Promise.all(
        auditEntries.slice(Number(offset), Number(offset) + Number(limit)).map(async (therapist) => {
          const profile = await storage.getTherapistProfileByUserId(therapist.id);
          return {
            therapistId: therapist.id,
            therapistName: `${therapist.firstName} ${therapist.lastName}`,
  ...
  app.delete('/api/therapists/:id/workspace-account', isAuthenticated, adminLimiter, ipSecurityCheck, async (req, res) => {
    try {
      const { id: therapistId } = req.params;
  ...
  app.get('/api/admin/workspace-service-health', isAuthenticated, adminLimiter, async (req, res) => {
    try {
