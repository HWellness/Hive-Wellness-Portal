...
import { stripe, processTherapistPayout, checkTherapistAccountStatus } from './stripe-revenue-split';
...
export class TherapistPayoutService {
  ...
  static async processSessionPayout(
    ...
    try {
      ...
      const accountValidation = await this.validateTherapistAccount(therapistStripeAccountId, auditTrail);
      ...
      if (transferResult.success) {
        try {
          ...
          console.log(`🛡️ [FINANCIAL SAFETY] Payment ${request.paymentId} marked as payout completed`);
      ...
      console.error(`❌ THERAPIST PAYOUT: Unexpected error processing session ${sessionId}:`, error);
  ...
  private static async validatePayoutRequest(
    ...
    const { sessionId, paymentId, therapistId, therapistStripeAccountId, therapistAmount } = request;
    ...
    try {
      ...
      try {
        ...
        if (paymentRecord.stripePaymentIntentId) {
          const paymentIntent = await stripe.paymentIntents.retrieve(paymentRecord.stripePaymentIntentId);
          ...
          if (paymentIntent.charges?.data?.[0]?.transfer) {
            ...
            auditTrail.push({
              ...
              metadata: { chargeId: paymentIntent.charges.data[0].id, transferId: existingTransfer }
            });
        ...
        console.warn(`⚠️ Could not verify Stripe PaymentIntent for double-payment check:`, stripeError);
      ...
      const expectedTherapistAmount = Math.round(parseFloat(paymentRecord.amount) * 0.85 * 100) / 100;
  ...
  private static async processStripeTransferWithRetry(
    ...
    while (retryCount <= retryOptions.maxRetries) {
      try {
        ...
        retryCount++;
        lastError = transferError instanceof Error ? transferError.message : 'Unknown transfer error';
  ...
  private static async createOrUpdatePayoutRecord(
    ...
    console.log(`🛡️ [RACE PROTECTION] UPSERT payout record for session ${request.sessionId}, payment ${request.paymentId}`);
    ...
    try {
      ...
      const isNewRecord = payoutRecord.createdAt && payoutRecord.updatedAt && 
      ...
      if (error && typeof error === 'object' && 'code' in error && error.code === '23505') {
        console.log(`🛡️ [RACE PROTECTION] Unique constraint prevented duplicate - fetching existing record`);
        
        // Fetch the existing record that caused the constraint violation
        const existingPayout = await this.getExistingPayoutRecord(request.sessionId, request.paymentId);
        if (existingPayout) {
          auditTrail.push({
            timestamp: new Date(),
      ...
      throw new Error(`Failed to create/update payout record: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  
  private static async getExistingPayoutRecord(sessionId: string, paymentId: string): Promise<any | null> {
    try {
...
export async function triggerTherapistPayoutOnSessionCompletion(
  ...
  try {
    ...
    if (!therapistProfile || !therapistProfile.stripeConnectAccountId) {
      throw new Error(`Therapist profile or Stripe account not found: ${appointment.primaryTherapistId}`);
    }
