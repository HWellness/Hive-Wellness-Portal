 ...
 */
export async function checkFreeSessionEligibility(userId: string): Promise<FreeSessionEligibility> {
  try {
    // Get user details
...
export async function markFreeSessionAsUsed(
  ...
  try {
    ...
    console.log(
      `✅ Marked free session as used for client ${userId}, appointment ${appointmentId}`
    );
 ...
 */
export function calculateSessionPayment(sessionFee: number, isFreeSession: boolean) {
...
export async function getFreeSessionStats() {
  try {
    ...
    const usageRate =
      totalClients.length > 0 ? (totalFreeSessionsUsed.length / totalClients.length) * 100 : 0;
