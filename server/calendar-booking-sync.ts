...
export class CalendarBookingSync {
  ...
  async testCalendarAccess() {
    try {
      ...
      console.log(`✅ Successfully accessed calendar: ${this.calendarId} (found ${testResponse.data.items?.length || 0} events)`);
      console.log('🎯 Using single calendar source for consistent booking results');
    } catch (error: any) {
      console.log(`❌ Cannot access calendar: ${this.calendarId} - ${error.message} (Code: ${error.code})`);
    }
  }
  ...
  async isTimeSlotAvailable(startTime: Date, endTime: Date): Promise<boolean> {
    try {
      ...
      console.log(`📏 Policy settings: Buffer=${this.bufferMinutes}min, Fail-secure=${this.failSecure}, Allowed days=${Array.from(this.allowedDays).join(',')}`);
      ...
      if (!this.allowedDays.has(dayOfWeek)) {
        ...
        const allowedDayNames = Array.from(this.allowedDays).map(d => dayNames[d]).join(', ');
        console.log(`🚫 Business rule: ${dayNames[dayOfWeek]} is not an available day (allowed: ${allowedDayNames})`);
        return false;
      }
      ...
      if (!eventResult.ok) {
        if (this.failSecure) {
          console.error(`🚫 FAIL-SECURE: Calendar error - BLOCKING booking for safety: ${eventResult.error}`);
          return false; // Fail-secure: treat errors as unavailable
        } else {
          console.warn(`⚠️ FAIL-OPEN: Calendar error - ALLOWING booking despite error: ${eventResult.error}`);
      ...
      const conflicts = events.filter((event: any) => {
        ...
        if (hasOverlap) {
          console.log(`⚠️ TIMED CONFLICT (with ${this.bufferMinutes}min buffer): ${event.summary || 'Blocked'} (${eventStart.toISOString()} - ${eventEnd.toISOString()})`);
        }
  ...
  async getAvailableSlots(date: Date, startHour: number = 9, endHour: number = 17, slotDurationMinutes: number = 30): Promise<Array<{start: Date, end: Date}>> {
    try {
      console.log(`🔍 Getting available slots for ${date.toDateString()} (${startHour}:00-${endHour}:00)`);
      console.log(`📊 Buffer: ${this.bufferMinutes}min, Allowed days: ${Array.from(this.allowedDays).join(',')}, Fail-secure: ${this.failSecure}`);
      ...
      if (!eventResult.ok) {
        if (this.failSecure) {
          console.error(`🚫 FAIL-SECURE: Calendar error - returning empty slots for safety: ${eventResult.error}`);
          return []; // Fail-secure: treat errors as no available slots
        } else {
          console.warn(`⚠️ FAIL-OPEN: Calendar error - ignoring error and proceeding (RISKY): ${eventResult.error}`);
      ...
      while (currentSlot < endOfDay) {
        ...
        if (slotEnd <= endOfDay) {
          ...
          const isBlocked = blockingPeriods.some(period => 
            currentSlot < period.end && slotEnd > period.start
          );
      ...
      if (this.failSecure) {
        ...
        console.warn('⚠️ FAIL-OPEN: Unexpected error - attempting to continue despite error (VERY RISKY)');
  ...
  async getBatchEventsForDay(startOfDay: Date, endOfDay: Date): Promise<any[]> {
    try {
      ...
      if (!eventResult.ok) {
        if (this.failSecure) {
          console.error(`🚫 FAIL-SECURE: Calendar error - returning empty events for safety: ${eventResult.error}`);
          return []; // Fail-secure: treat errors as no events
        } else {
          console.warn(`⚠️ FAIL-OPEN: Calendar error - returning empty events despite error: ${eventResult.error}`);
      ...
      if (process.env.NODE_ENV !== 'production' && events.length > 0) {
        console.log('📝 Day events summary:', events.map((e: any) => ({
  ...
  async getBlockedTimes(startDate: Date, endDate: Date): Promise<Array<{start: Date, end: Date, summary: string}>> {
    try {
      if (!this.calendar) {
        return [];
      }

      console.log(`🔍 Getting blocked times from ${startDate.toDateString()} to ${endDate.toDateString()}`);
      ...
      while (currentDate <= endDate) {
        ...
        if (!eventResult.ok) {
          if (this.failSecure) {
            console.error(`🚫 FAIL-SECURE: Calendar error for ${currentDate.toDateString()} - skipping day: ${eventResult.error}`);
            // Skip this day in fail-secure mode
          } else {
            console.warn(`⚠️ FAIL-OPEN: Calendar error for ${currentDate.toDateString()} - skipping day: ${eventResult.error}`);
      ...
      console.error('❌ SECURITY: Failed to get blocked times - returning empty for safety:', error?.message || error);
  ...
  async checkAvailabilityWithDetails(startTime: Date, endTime: Date): Promise<{
    available: boolean;
    reason?: string;
    ...
    try {
      ...
      if (!eventResult.ok) {
        if (this.failSecure) {
          ...
          console.warn(`⚠️ FAIL-OPEN: Calendar error - treating as available (RISKY): ${eventResult.error}`);
  ...
  async syncAvailabilityToCalendar(availability: Array<{day: number, startTime: string, endTime: string}>): Promise<boolean> {
    try {
      if (!this.calendar) {
  ...
  private async createBlockingEventsForDay(dayOfWeek: number, availableStart: string, availableEnd: string): Promise<void> {
    ...
    if (startHour > 0 || startMin > 0) {
      ...
      await this.createSingleBlockingEvent(morningStart, morningEnd, `🚫 Unavailable (Before ${availableStart})`);
    }
    ...
    if (endHour < 23 || endMin < 59) {
      ...
      await this.createSingleBlockingEvent(eveningStart, eveningEnd, `🚫 Unavailable (After ${availableEnd})`);
    }
  }

  private async createSingleBlockingEvent(startTime: Date, endTime: Date, summary: string): Promise<void> {
    try {
      const event = {
        summary,
        start: {
   ...
   */
  async checkUnifiedAvailability(startTime: Date, endTime: Date): Promise<{
    available: boolean;
    reason?: string;
    policies: {
      allowedDays: boolean;
    ...
    console.log(`📋 Active policies: Buffer=${this.bufferMinutes}min, Days=[${Array.from(this.allowedDays).join(',')}], Fail-secure=${this.failSecure}`);
...
export class AvailabilityService {
  ...
  static async getSlots(
    ...
    const result = await this.instance.getUnifiedAvailableSlots(date, startHour, endHour, slotDurationMinutes);
    return result.slots;
  }
  ...
  static async getDetailedSlots(
    ...
    return await this.instance.getUnifiedAvailableSlots(date, startHour, endHour, slotDurationMinutes);
  }
