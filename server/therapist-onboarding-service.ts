...
import { GoogleWorkspaceAdminService, type TherapistAccountData } from './google-workspace-admin-service';
...
export class TherapistOnboardingService {
    ...
    try {
      ...
      for (const enquiry of approvedEnquiries) {
        try {
          ...
          const existingApplication = await this.storage.getTherapistOnboardingApplicationByEmail(enquiry.email);
          ...
          console.log(`✅ Created onboarding application for ${enquiry.email} (ID: ${application.id})`);
      ...
      console.log(`🎯 Conversion complete: ${converted}/${approvedEnquiries.length} enquiries converted to applications`);
  ...
  async processTherapistApproval(enquiryId: string): Promise<OnboardingWorkflowResult> {
    ...
    try {
      ...
      console.log(`📋 Processing approval for ${enquiry.firstName} ${enquiry.lastName} (${enquiry.email})`);
      ...
      if (userResult.success && userResult.userId) {
        try {
          ...
          if (calendarSetupResult.success) {
            ...
            console.log(`✅ Calendar setup completed for therapist ${userResult.userId}: ${calendarSetupResult.googleCalendarId}`);
          } else {
            console.warn(`⚠️ Calendar setup failed for therapist ${userResult.userId}: ${calendarSetupResult.error}`);
            result.details.calendarSetupError = calendarSetupResult.error;
          }
        } catch (calendarError: any) {
          console.error(`❌ Calendar setup error for therapist ${userResult.userId}:`, calendarError);
      ...
      console.log(`📊 Results: Workspace: ${result.workspaceProvisioned}, User: ${result.userAccountCreated}, Calendar: ${result.calendarSetupCompleted}`);
      return result;
  ...
  async provisionGoogleWorkspaceAccount(enquiry: any): Promise<TherapistProvisioningResult> {
    try {
      console.log(`📧 Provisioning Google Workspace account for ${enquiry.firstName} ${enquiry.lastName}`);
      ...
      if (workspaceResult.accountCreated) {
        ...
        const workspaceUpdateData = {
          ...
          workspaceTempPassword: workspaceResult.tempPassword ? await bcrypt.hash(workspaceResult.tempPassword, 10) : null,
   ...
   */
  async createTherapistUserAccount(enquiry: any, workspaceResult: TherapistProvisioningResult): Promise<{
    success: boolean;
    userId?: string;
    error?: string;
  }> {
    try {
      ...
      const profileData = {
        ...
        workspaceAccountStatus: workspaceResult.success ? 'active' as const : 'pending' as const,
        workspaceTempPassword: workspaceResult.tempPassword ? await bcrypt.hash(workspaceResult.tempPassword, 10) : null,
        calendarPermissionsConfigured: workspaceResult.success || false,
        workspaceAccountNotes: workspaceResult.success ? `Account auto-provisioned on ${new Date().toISOString()}` : null,
        createdAt: new Date(),
        updatedAt: new Date()
      };
   ...
   */
  async sendTherapistWelcomeEmail(enquiry: any, workspaceResult: TherapistProvisioningResult, calendarSetupResult?: any): Promise<void> {
    try {
      const emailData = {
        to: enquiry.email,
      ...
      console.log(`📧 Welcome email sent to ${enquiry.email} (includes calendar setup: ${calendarSetupResult?.success ? 'Yes' : 'No'})`);
...
export const createTherapistOnboardingService = (storage: Storage) => new TherapistOnboardingService(storage);