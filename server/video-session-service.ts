...
export class VideoSessionService {
  ...
  static async getVideoSession(sessionId: string): Promise<VideoSessionData | null> {
    try {
      ...
      const appointmentRows = await db.select().from(appointments).where(eq(appointments.id, sessionId));
      ...
      if (appointmentRows.length > 0) {
        ...
        const [clientRows, therapistRows] = await Promise.all([
          appointment.clientId ? db.select().from(users).where(eq(users.id, appointment.clientId)) : Promise.resolve([]),
          appointment.primaryTherapistId ? db.select().from(users).where(eq(users.id, appointment.primaryTherapistId)) : Promise.resolve([])
        ]);
        ...
        if (!meetingUrl) {
          ...
          if (client && therapist) {
            try {
              ...
              console.error(`❌ Failed to create Daily.co room for session ${sessionId}:`, roomError);
              console.log(`⚠️ Session ${sessionId} will be returned without meeting URL - Daily.co integration temporarily unavailable`);
            ...
            console.error(`❌ Cannot create Daily.co room - missing client or therapist data for appointment ${sessionId}`);
            console.log(`⚠️ Session ${sessionId} will be returned without meeting URL - missing participant information`);
            meetingUrl = null;
          }
        } else {
        ...
        const sessionData: VideoSessionData = {
          ...
          participants: [
            { id: client?.id || '', name: `${client?.firstName || ''} ${client?.lastName || ''}`.trim(), role: 'client' },
            { id: therapist?.id || '', name: `${therapist?.firstName || ''} ${therapist?.lastName || ''}`.trim(), role: 'therapist' }
          ],
          joinInstructions: `
...
- Duration: ${appointment.duration || 50} minutes
      ...
      const introCalls = await db.select().from(introductionCalls).where(eq(introductionCalls.id, sessionId));
      ...
      if (introCalls.length > 0) {
        ...
        if (!meetingUrl) {
          ...
          try {
            ...
            await db.update(introductionCalls)
              .set({
            ...
            console.error(`❌ Failed to create Daily.co room for introduction call ${sessionId}:`, roomError);
            meetingUrl = null;
          }
        } else {
...
Session starts at: ${new Date(introCall.preferredDate).toLocaleString('en-GB', { 
      ...
      console.log(`VideoSessionService: Session ${sessionId} not found in database, checking demo sessions...`);
      ...
      if (demoSession) {
        ...
        const sessionData: VideoSessionData = {
          ...
          participants: [
            { id: demoSession.clientId, name: demoSession.clientName || 'Demo Client', role: 'client' },
            { id: demoSession.therapistId, name: demoSession.therapistName || 'Demo Therapist', role: 'therapist' }
          ],
          joinInstructions: `
...
- Duration: ${demoSession.duration} minutes
      ...
      console.log(`VideoSessionService: Session ${sessionId} not found in appointments or introduction calls`);
      return null;
   ...
   */
  static async getUserVideoSessions(userId: string, userEmail?: string): Promise<VideoSessionData[]> {
    try {
      console.log(`VideoSessionService: Getting video sessions for user ${userId}, email: ${userEmail}`);
      
      const sessions: VideoSessionData[] = [];
      
      // Get appointments where user is either client or therapist
      const appointmentRows = await db.select().from(appointments).where(
        or(
          eq(appointments.clientId, userId),
          eq(appointments.primaryTherapistId, userId)
        )
      );
      ...
      for (const appointment of appointmentRows) {
        ...
        if (!meetingUrl) {
          ...
          console.log(`VideoSessionService: No Daily.co URL for appointment ${appointment.id}, session may be pending setup`);
      ...
      let introCalls: any[] = [];
      if (userEmail) {
        introCalls = await db.select().from(introductionCalls).where(eq(introductionCalls.email, userEmail));
        console.log(`VideoSessionService: Found ${introCalls.length} introduction calls for email ${userEmail}`);
      } else {
      ...
      for (const introCall of introCalls) {
        ...
        if (!meetingUrl) {
          ...
          console.log(`VideoSessionService: No meeting URL for introduction call ${introCall.id}, call may be pending setup`);
          meetingUrl = ''; // Empty URL - will be created when user joins
        }
      ...
      throw new Error(`Failed to get user video sessions: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  static async joinVideoSession(sessionId: string, userId: string): Promise<{ success: boolean; meetingUrl: string }> {
    try {
      ...
      console.log(`✅ User ${userId} successfully joined session ${sessionId} with meeting URL: ${meetingUrl || 'none'}`);
      return {
        success: true,
        meetingUrl: meetingUrl || ''
      };
      ...
      throw new Error(`Failed to join video session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  static async leaveVideoSession(sessionId: string, userId: string, options?: { 
    ...
    try {
      ...
      if (options?.autoComplete) {
        ...
        if (sessionData && sessionData.sessionType === 'therapy' && 
          ...
          if (sessionDuration >= minimumSessionDuration) {
            console.log(`⏰ Session ${sessionId} ran for ${sessionDuration.toFixed(1)} minutes - triggering automatic completion with payment`);
            ...
            try {
              ...
              console.log(`🚀 Triggering automatic session completion with payment for session ${sessionId}`);
              
              paymentResult = await SessionPaymentService.completeSessionWithPayment(completionData, {
              ...
              if (paymentResult.success) {
                console.log(`✅ Session ${sessionId} automatically completed with payment processing initiated`);
              } else {
                console.log(`⚠️ Session ${sessionId} completion attempted but payment processing failed: ${paymentResult.error}`);
                requiresManualCompletion = true;
              }
              
            } catch (autoCompletionError) {
              console.error(`❌ Automatic session completion failed for ${sessionId}:`, autoCompletionError);
              requiresManualCompletion = true;
              
              // Fallback: Mark session as requiring manual completion
              console.log(`🔄 Session ${sessionId} marked for manual completion due to auto-completion failure`);
            }
            
          } else {
            console.log(`⏰ Session ${sessionId} only ran ${sessionDuration.toFixed(1)} minutes - below minimum ${minimumSessionDuration} minutes for billing`);
            ...
            try {
              ...
              console.log(`✅ Session ${sessionId} marked as completed without payment (duration too short)`);
            } catch (error) {
      ...
      throw new Error(`Failed to leave video session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private static async markSessionCompletedWithoutPayment(sessionId: string, reason: string): Promise<void> {
    try {
