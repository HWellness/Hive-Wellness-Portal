...
async function setupTwilioIntegration(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `✅ Account SID: ${config.accountSid ? `${config.accountSid.substring(0, 8)}...` : "❌ Missing"}`
    );
    ...
    console.log(
      `✅ WhatsApp Number: ${config.whatsappNumber ? config.whatsappNumber : "❌ Missing"}`
    );

    if (!config.accountSid || !config.authToken || !config.fromNumber) {
      throw new Error(
        "Missing required Twilio credentials. Please configure TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER"
      );
    }
    ...
    if (!isInitialized) {
      throw new Error(
        "Twilio service failed to initialize. Check credentials and network connectivity."
      );
    }
    ...
    if (config.whatsappNumber) {
      ...
      console.log(
        `   □ Webhook URL: ${process.env.BASE_URL || "https://your-domain.com"}/api/twilio/webhook`
      );
    } else {
    ...
    if (process.env.NODE_ENV === "development" && process.env.TEST_NOTIFICATIONS === "true") {
      ...
      if (testRecipient) {
        ...
        console.log(
          `✅ Test SMS Result: ${testResult.success ? "Sent successfully" : `Failed - ${testResult.error}`}`
        );
    ...
    console.log(
      `TWILIO_WHATSAPP_NUMBER=${config.whatsappNumber || "Using SMS number as fallback"}`
    );
