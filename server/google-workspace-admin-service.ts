...
export class GoogleWorkspaceAdminService {
   ...
   */
  private async checkCalendarExists(
    userEmail: string,
    calendarName: string
  ): Promise<string | null> {
    try {
   ...
   */
  async createTherapistAccount(
    therapistData: TherapistAccountData
  ): Promise<TherapistWorkspaceResult> {
    try {
      console.log(
        `🚀 Creating therapist account for: ${therapistData.firstName} ${therapistData.lastName}`
      );
      ...
      if (userExists) {
        ...
        const existingCalendarId = await this.checkCalendarExists(
          workspaceEmail,
          `Dr. ${therapistData.firstName} ${therapistData.lastName}`
        );
        return {
          workspaceEmail,
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "⚠️ Failed to create cost tracking record, but account creation succeeded:",
            costTrackingError
          );
        ...
        console.error(
          `❌ Error during account creation for ${workspaceEmail}, attempting rollback:`,
          error
        );
    ...
    try {
      ...
      const userCalendars =
        calendarList.data.items
          ?.filter((cal: any) => cal.id?.includes(userEmail.split("@")[0]) || cal.id === userEmail)
          .map((cal: any) => cal.id) || [];
  ...
  private async createWorkspaceAccountRecord(
    ...
    try {
      ...
      const monthlyCostGBP = workspaceCostsGBP[planType as keyof typeof workspaceCostsGBP] || 9.6;
      const monthlyCostUSD = workspaceCostsUSD[planType as keyof typeof workspaceCostsUSD] || 12.0;
      ...
      console.log(
        `💰 Created cost tracking records for therapist ${existingUser.id} with workspace ${workspaceEmail}`
      );
    } catch (error) {
