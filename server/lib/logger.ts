...
if (!process.env.LOG_HASH_SALT) {
  const errorMsg =
    "❌ CRITICAL SECURITY ERROR: LOG_HASH_SALT environment variable is required for PII sanitization";
  console.error(errorMsg);
  console.error(
    "🔐 HIPAA Compliance: The hash salt must be a secret value to protect patient data"
  );
  console.error(
    "📋 Add LOG_HASH_SALT to your environment variables before starting the application"
  );
  ...
  if (typeof value === "object") {
    ...
    for (const [key, val] of Object.entries(value)) {
      ...
      // Hash email fields
        ...
        sanitized[key] =
          typeof val === "string" ? hashIdentifier(val, "EMAIL") : sanitizeValue(val, depth + 1);
      }
      // Hash phone fields
      else if (PHONE_FIELDS.some((field) => lowerKey === field.toLowerCase())) {
        sanitized[key] =
          typeof val === "string" ? hashIdentifier(val, "PHONE") : sanitizeValue(val, depth + 1);
      }
      // Hash name fields
      else if (NAME_FIELDS.some((field) => lowerKey === field.toLowerCase())) {
        sanitized[key] =
          typeof val === "string" ? hashIdentifier(val, "NAME") : sanitizeValue(val, depth + 1);
      }
