...
export class DailyService {
    ...
    try {
      ...
      if (existingRoom) {
        ...
        if (existingRoom.privacy === roomConfig.privacy) {
          ...
          console.log(`🗑️ Deleting existing room with wrong privacy (${existingRoom.privacy} vs ${roomConfig.privacy})`);
      ...
      throw new Error(`Failed to create video room: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
    ...
    try {
      ...
      if (existingRoom) {
        ...
        if (existingRoom.privacy === roomConfig.privacy) {
          ...
          // Delete existing room with wrong privacy setting
          console.log(`🗑️ Deleting existing admin call room with wrong privacy (${existingRoom.privacy} vs ${roomConfig.privacy})`);
          await this.deleteRoom(roomName);
        }
      }
      ...
      throw new Error(`Failed to create admin call room: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  async createMeetingToken(roomName: string, participantData: {
    userId: string;
    userName: string;
    isOwner?: boolean;
  }): Promise<string | null> {
