...
export async function verifyJordanImport(storage: any) {
  ...
  try {
    ...
    const jordanQuestionnaires = questionnaires.filter(
      (q: any) => q.step2Email?.includes("jordan") || q.step2FirstName?.toLowerCase() === "jordan"
    );
    ...
    const realJordan = jordanQuestionnaires.find(
      (q: any) => q.step2Email === "jordan@realclient.com" && q.step4Gender === "Non-binary"
    );
