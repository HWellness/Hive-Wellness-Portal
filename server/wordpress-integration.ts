...
class WordPressIntegration {
  ...
  async getNewEntries(formId: string): Promise<GravityFormEntry[]> {
    try {
      ...
      if (this.config.lastPolled) {
        ...
        searchParams.append('search', JSON.stringify({
  ...
  async processEntry(entry: GravityFormEntry, form: GravityForm): Promise<void> {
    ...
    const entryData = {
      ...
      therapist_application: this.extractField(entry, ['therapist_application', 'application_type', '8']),
  ...
  private async generateSignature(method: string, route: string, expires: number): Promise<string> {
    ...
    const hash = crypto.createHmac('sha1', this.config.privateApiKey).update(stringToSign).digest('base64');
  ...
  private async makeApiRequest(endpoint: string, method: string = 'GET'): Promise<any> {
    ...
    const route = endpoint.startsWith('forms') || endpoint.startsWith('entries') ? endpoint : `forms${endpoint}`;
    ...
    console.log('🔗 Making Gravity Forms API v1 request to:', finalUrl.replace(/signature=[^&]+/, 'signature=***'));
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error(`Authentication failed: Invalid API keys or signature. Please verify your Public Key: ${this.config.publicApiKey} and Private Key in WordPress Gravity Forms > Settings > REST API. (${response.status})`);
      } else if (response.status === 404) {
        throw new Error(`API endpoint not found: The URL ${url} was not found. Please ensure Gravity Forms plugin is installed, REST API v1 is enabled, and pretty permalinks are configured. (${response.status})`);
      } else {
        throw new Error(`Gravity Forms API request failed: ${response.status} ${response.statusText} - ${errorText}`);
      }
    }
    ...
    if (responseText.trim().startsWith('<!DOCTYPE') || responseText.trim().startsWith('<html')) {
      ...
      if (responseText.includes('Coming Soon') || responseText.includes('maintenance') || responseText.includes('Under Construction')) {
    ...
    try {
      ...
      throw new Error(`Failed to parse API response as JSON. Response was: ${responseText.substring(0, 200)}...`);
    }
    ...
    if (jsonResponse.status && jsonResponse.status >= 400) {
      throw new Error(`API Error ${jsonResponse.status}: ${jsonResponse.response || 'Unknown error'}`);
    }
