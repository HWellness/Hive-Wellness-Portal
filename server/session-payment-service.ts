...
export class SessionPaymentService {
  ...
  static async completeSessionWithPayment(
    ...
    console.log(
      `🏁 SessionPaymentService: Starting session completion for ${sessionId} by ${userId} (${userRole})`
    );
    ...
    try {
      ...
      if (existingPayment) {
        console.log(
          `⚠️ Payment already exists for session ${sessionId}: ${existingPayment.status}`
        );
      ...
      if (paymentResult.success) {
        console.log(
          `✅ Payment processed successfully for session ${sessionId}: ${paymentResult.paymentId}`
        );
        ...
        console.error(
          `❌ Payment processing failed for session ${sessionId}: ${paymentResult.error}`
        );
      }
      ...
      console.error(
        `❌ SessionPaymentService: Unexpected error completing session ${sessionId}:`,
        error
      );
    ...
    try {
      ...
      if (sessionDetails.sessionType === "therapy") {
        const appointmentResult = await db
          ...
          .leftJoin(
            therapistProfiles,
            eq(appointments.primaryTherapistId, therapistProfiles.userId)
          )
  ...
  private static async processSessionPayment(params: {
    ...
    try {
      ...
      const isFreeSession =
        sessionPrice === 0 &&
        client.freeSessionUsed &&
        client.freeSessionAppointmentId === sessionId;
      ...
      const accountStatus = await checkTherapistAccountStatus(
      ...
      if (isFreeSession) {
        console.log(
          `🎁 Processing FREE first session for ${client.email} - paying therapist from platform funds`
        );
        ...
        console.log(
          `✅ Free session: Therapist paid £${therapistAmount} from platform funds (transfer: ${transferId})`
        );
      ...
      console.log(
        `💳 Creating payment for session ${sessionId}: £${sessionFee} (${paymentTiming})`
      );
  ...
  static async confirmSessionPayment(
    ...
    console.log(
      `💳 ENHANCED CONFIRMATION: Confirming payment ${paymentIntentId} for session ${sessionId} by user ${userId}`
    );
    ...
    try {
      ...
      if (confirmationResult.success) {
        console.log(
          `🎉 ENHANCED CONFIRMATION SUCCESS: Session ${sessionId} payment confirmed after ${confirmationResult.attempts} attempts`
        );
      } else {
        console.log(
          `❌ ENHANCED CONFIRMATION FAILED: Session ${sessionId} payment failed after ${confirmationResult.attempts} attempts: ${confirmationResult.lastError}`
        );
      }
  ...
  private static mapPaymentStatus(
    status: "succeeded" | "failed" | "pending" | "requires_action" | "cancelled"
    ...
    try {
      ...
      if (paymentId) {
        ...
        if (paymentRecord && paymentRecord.payoutCompleted) {
          console.log(
            `🛡️ [RACE PROTECTION] Payout already completed for payment ${paymentId} - skipping duplicate trigger`
          );
        } else {
          // ELEMENT #5: Trigger automated therapist payout for completed session
          console.log(
            `💰 ELEMENT #5: Triggering therapist payout for session ${sessionId} with payment ${paymentId}`
          );
          ...
          try {
            ...
            if (payoutResult.success) {
              console.log(
                `✅ ELEMENT #5: Therapist payout ${payoutResult.payoutId} triggered successfully for session ${sessionId}`
              );
            } else {
              console.error(
                `❌ ELEMENT #5: Therapist payout failed for session ${sessionId}: ${payoutResult.message}`
              );
            ...
            console.error(
              `❌ ELEMENT #5: Unexpected error triggering therapist payout for session ${sessionId}:`,
              payoutError
            );
   ...
   */
  static async retryFailedSessionPayment(
    sessionId: string,
    adminUserId: string
  ): Promise<SessionPaymentResult> {
    ...
    try {
      ...
      if (
        existingPayment &&
        (existingPayment.status === "succeeded" || existingPayment.status === "completed")
      ) {
        return {
          success: true,
          paymentId: existingPayment.id,
