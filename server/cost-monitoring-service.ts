...
import { currencyService, type SupportedCurrency, type CurrencyAmount, WORKSPACE_PLAN_COSTS_GBP, OVERAGE_COSTS_GBP } from "./currency-service";
...
export interface OptimizationRecommendation {
  id: string;
  type: 'low-utilization' | 'plan-downgrade' | 'storage-cleanup' | 'api-optimization' | 'license-consolidation';
...
export class GoogleWorkspaceCostCalculator {
   ...
   */
  async calculateTherapistCostBreakdown(therapistId: string, month?: string): Promise<TherapistCostBreakdown> {
    ...
    const [usage] = await db
      ...
      .where(
        and(
          ...
          eq(usageMetrics.month, targetMonth)
        )
      )
      .limit(1);
    ...
    const baseCostAmount = workspaceCostsGBP[workspaceAccount.planType as keyof typeof workspaceCostsGBP] || 9.60;
    ...
    const totalMonthlyCostAmount = baseCostAmount + storageOverageCostAmount + apiOverageCostAmount + meetRecordingCostAmount;
    const costPerAppointmentAmount = appointmentsCount > 0 ? totalMonthlyCostAmount / appointmentsCount : totalMonthlyCostAmount;
    ...
    const efficiencyScore = this.calculateEfficiencyScore(costPerAppointmentAmount, utilizationRate, storageUsedGB, storageQuotaGB);
  ...
  async calculateSystemCostSummary(month?: string): Promise<SystemCostSummary> {
    ...
    const totalCostAmount = therapistBreakdowns.reduce((sum, breakdown) => sum + breakdown.totalMonthlyCost.amount, 0);
    const totalWorkspaceCostAmount = therapistBreakdowns.reduce((sum, breakdown) => sum + breakdown.baseCost.amount, 0);
    const totalOverageCostsAmount = totalCostAmount - totalWorkspaceCostAmount;
    const totalAppointments = therapistBreakdowns.reduce((sum, breakdown) => sum + breakdown.appointmentsCount, 0);
    const averageCostPerTherapistAmount = activeAccounts.length > 0 ? totalCostAmount / activeAccounts.length : 0;
    const costPerAppointmentAmount = totalAppointments > 0 ? totalCostAmount / totalAppointments : 0;
    const averageUtilization = activeAccounts.length > 0 
      ? therapistBreakdowns.reduce((sum, breakdown) => sum + breakdown.utilizationRate, 0) / activeAccounts.length 
      : 0;
    const averageEfficiency = activeAccounts.length > 0
      ? therapistBreakdowns.reduce((sum, breakdown) => sum + breakdown.efficiencyScore, 0) / activeAccounts.length
      : 0;
    ...
    const monthOverMonthChange = previousMonthReport 
      ? ((totalCostAmount - parseFloat(previousMonthReport.totalCost.toString())) / parseFloat(previousMonthReport.totalCost.toString())) * 100
      : 0;

    const yearOverYearChange = previousYearReport
      ? ((totalCostAmount - parseFloat(previousYearReport.totalCost.toString())) / parseFloat(previousYearReport.totalCost.toString())) * 100
      : 0;
    ...
    const projectedNextMonthCostAmount = this.projectNextMonthCosts(therapistBreakdowns, monthOverMonthChange);
    ...
    const optimizationPotentialAmount = therapistBreakdowns.reduce(
      (sum, breakdown) => sum + breakdown.recommendations.reduce(
        (recSum, rec) => recSum + rec.potentialMonthlySavings.amount, 0
      ), 0
    );
    ...
    return {
      ...
      totalAppointments,
      averageCostPerTherapist: currencyService.createCurrencyAmount(averageCostPerTherapistAmount, 'GBP'),
      costPerAppointment: currencyService.createCurrencyAmount(costPerAppointmentAmount, 'GBP'),
      ...
      topCostDrivers,
      projectedNextMonthCost: currencyService.createCurrencyAmount(projectedNextMonthCostAmount, 'GBP'),
      budgetVariance: currencyService.createCurrencyAmount(budgetVarianceAmount, 'GBP'),
      optimizationPotential: currencyService.createCurrencyAmount(optimizationPotentialAmount, 'GBP')
    };
  }
  ...
  async generateCostProjections(
    ...
    const averageCostPerTherapist = currentCount > 0 ? currentMonthlyCost / currentCount : 12.00;
  ...
  private async generateOptimizationRecommendations(data: any): Promise<OptimizationRecommendation[]> {
    ...
    if (data.workspaceAccount.planType === 'business-plus' && data.utilizationRate < 60) {
      recommendations.push({
        ...
        potentialMonthlySavings: WORKSPACE_PLAN_COSTS['business-plus'] - WORKSPACE_PLAN_COSTS['business-standard'],
  ...
  private identifyTopCostDrivers(breakdowns: TherapistCostBreakdown[]): CostDriver[] {
    ...
    const overageCostTotal = breakdowns.reduce((sum, b) => sum + b.storageOverageCost + b.apiOverageCost, 0);
  ...
  private shouldSendAlert(budget: CostBudget, threshold: number): boolean {
    ...
    return !budget.lastAlertSent || 
           (Date.now() - budget.lastAlertSent.getTime()) > (24 * 60 * 60 * 1000); // 24 hours
  }
}
