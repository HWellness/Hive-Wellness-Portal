...
import { db } from "./db";
import type { WebhookEvent, InsertWebhookEvent, InsertWebhookProcessingQueue } from "@shared/schema";
...
export class WebhookProcessor {
   ...
   */
  async processStripeWebhook(event: any, webhookId: string): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (!wasCreated) {
        ...
        if (webhookEvent.processingStatus === 'completed') {
          ...
          secureLogger.log(`⚠️ [${webhookId}] Event ${eventId} is currently being processed - accepting as success`);
   ...
   */
  private async processEventByType(event: any, webhookId: string, webhookEventId: string): Promise<{
    appointmentId?: string;
    ...
    try {
      switch (event.type) {
        case 'payment_intent.succeeded':
          ...
          const result = await this.processPaymentSucceeded(event.data.object, webhookId, webhookEventId);
   ...
   */
  private async processPaymentSucceeded(paymentIntent: any, webhookId: string, webhookEventId: string): Promise<{
    appointmentId?: string;
    operationsCompleted: string[];
  }> {
  ...
  private shouldCreateAppointment(metadata: any): boolean {
    ...
    const isNewBooking = metadata.booking_source === 'new' || 
  ...
  private async createAppointmentAtomically(
    ...
    return await db.transaction(async (tx) => {
      ...
      console.log(`📅 [${webhookId}] Deferring Google Meet room creation to async worker to prevent DB timeout`);
      
      if (isPastDate || isBackdated) {
        console.log(`🔙 [${webhookId}] SERVICE GUARD: Past/backdated appointment - Meet URL will remain empty`, {
          scheduledAt: scheduledAt.toISOString(),
          isPastDate,
          isBackdated,
      ...
      try {
        ...
        console.log(`🧑‍⚕️ [${webhookId}] Appointment therapist ID: ${appointment.primaryTherapistId}`);
        ...
        if (appointmentError.code === 'APPOINTMENT_OVERLAP' || 
          ...
          console.warn(`⚠️ [${webhookId}] Appointment overlap detected - this is expected for webhook idempotency`);
          ...
          try {
            const therapistAppointments = await this.storage.getAppointmentsByTherapist(normalized.therapistId);
            const existingAppointments = therapistAppointments.filter(apt => {
              const aptStart = new Date(apt.scheduledAt);
              const aptEnd = new Date(apt.endTime || new Date(aptStart.getTime() + (apt.duration || 50) * 60000));
            ...
            if (exactMatch) {
              console.log(`🔍 [${webhookId}] Found exact matching appointment for idempotency check: ${exactMatch.id}`);
              
              // Verify this is the same payment intent (true idempotency)
              try {
                const existingPayment = await this.storage.getPaymentByStripePaymentIntentId(paymentIntent.id);
                if (existingPayment && existingPayment.appointmentId === exactMatch.id) {
                  console.log(`✅ [${webhookId}] True idempotent retry detected - appointment: ${exactMatch.id}`);
                  // TODO: Re-assert downstream queue items for completeness
                  return exactMatch.id;
                }
              } catch (paymentLookupError) {
                console.warn(`⚠️ [${webhookId}] Could not verify existing payment:`, paymentLookupError);
              }
              ...
              try {
                ...
                console.error(`❌ [${webhookId}] Failed to create payment for exact match:`, paymentError);
                throw appointmentError;
              }
            } else {
              // No exact match found - this is a genuine conflict with different parameters
              console.error(`❌ [${webhookId}] Genuine appointment conflict - no exact match found`);
              console.error(`❌ [${webhookId}] New booking conflicts with existing appointment(s) but doesn't match exactly`);
              throw appointmentError;
            }
          } catch (lookupError) {
          ...
          console.error(`❌ [${webhookId}] Overlap detected but no existing appointment found - this should not happen`);
          throw appointmentError;
        }
   ...
   */
  private async updateExistingAppointmentPayment(paymentIntent: any, webhookId: string): Promise<void> {
    const metadata = paymentIntent.metadata;
    ...
    try {
      ...
      console.log(`✅ [${webhookId}] Appointment ${appointmentId} marked as completed with payment`);
    } catch (appointmentUpdateError) {
      console.error(`❌ [${webhookId}] Failed to mark appointment complete:`, appointmentUpdateError);
    }

    // CRITICAL FIX: Trigger therapist payout (85% revenue split)
    if (paymentRecord) {
      try {
        console.log(`💰 [${webhookId}] Triggering therapist payout for payment ${paymentRecord.id} on appointment ${appointmentId}`);
        ...
        if (payoutResult.success) {
          console.log(`✅ [${webhookId}] Therapist payout ${payoutResult.payoutId} triggered successfully via webhook`);
        } else {
          console.error(`❌ [${webhookId}] Therapist payout failed via webhook: ${payoutResult.message}`);
          // Log but don't fail the webhook - payout can be retried
        }
      } catch (payoutError) {
        console.error(`❌ [${webhookId}] Unexpected error triggering therapist payout via webhook:`, payoutError);
        // Log but don't fail the webhook - payout can be retried manually
      }
    } else {
      console.warn(`⚠️ [${webhookId}] Could not trigger payout - payment record not found after update`);
    }
  }
   ...
   */
  private async processPaymentFailed(paymentIntent: any, webhookId: string, webhookEventId: string): Promise<{
    operationsCompleted: string[];
  }> {
  ...
  private async processQueueItem(item: any): Promise<void> {
    ...
    try {
      ...
      if (shouldRetry) {
        const nextRetryAt = new Date(Date.now() + this.RETRY_DELAYS[Math.min(item.currentRetries, this.RETRY_DELAYS.length - 1)]);
  ...
  private async createCalendarEvent(data: any): Promise<void> {
    ...
    if (appointment.googleMeetUrl && appointment.googleMeetUrl.trim() !== '') {
      console.log(`✅ Appointment ${appointmentId} already has Google Meet URL - skipping creation`);
      return;
    }
    ...
    if (isPastDate || isBackdated) {
      console.log(`🔙 SERVICE GUARD: Skipping Google Meet creation for past/backdated appointment ${appointmentId}`);
      ...
      try {
        ...
        console.log(`✅ Google Meet room created for appointment ${appointmentId}: ${googleMeetUrl}`);
        
      } catch (meetError) {
        console.warn(`⚠️ Failed to create Google Meet room for appointment ${appointmentId}, using fallback:`, meetError);
        ...
        try {
          ...
          console.log(`✅ Fallback Google Meet URL created for appointment ${appointmentId}: ${googleMeetUrl}`);
        } catch (fallbackError) {
          console.error(`❌ Fallback Google Meet creation also failed for appointment ${appointmentId}:`, fallbackError);
    ...
    try {
      ...
      console.error(`❌ Failed to update appointment ${appointmentId} with Google Meet URL:`, updateError);
  ...
  private async sendConfirmationEmail(data: any): Promise<void> {
    ...
    try {
      ...
      const [client, therapist] = await Promise.all([
        ...
        appointment.primaryTherapistId ? this.storage.getUserById(appointment.primaryTherapistId) : null
      ]);
      ...
      console.error(`❌ Error sending confirmation emails for appointment ${data.appointmentId}:`, error);
      throw error;
    }
  }
   ...
   */
  private async sendClientConfirmationEmail(appointment: any, client: any, therapist: any): Promise<void> {
    const mailService = new MailService();
    mailService.setApiKey(process.env.SENDGRID_API_KEY!);

    const sessionDate = new Date(appointment.scheduledAt);
    const meetingUrl = appointment.googleMeetLink || appointment.videoRoomId || 'https://meet.google.com/';
   ...
   */
  private async sendTherapistConfirmationEmail(appointment: any, client: any, therapist: any): Promise<void> {
    const mailService = new MailService();
    mailService.setApiKey(process.env.SENDGRID_API_KEY!);
  ...
  private async processQueueItemAtomic(item: any): Promise<void> {
    ...
    try {
      ...
      if (item.currentRetries < item.maxRetries && retryDelay) {
        ...
        console.log(`🔄 [${this.WORKER_ID}] Queue item ${itemId} scheduled for retry in ${retryDelay}ms`);
      } else {
