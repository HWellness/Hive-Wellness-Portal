...
test.describe('Therapist User Journey', () => {
  test('should login as therapist and access dashboard', async ({ page }) => {
    ...
    await page.locator('[data-testid="email-input"], input[type="email"]').fill('therapist@demo.hive');
    ...
    await expect(page.locator('text=/thompson|therapist|dashboard/i').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should access calendar and appointments', async ({ page }) => {
    ...
    const calendarLink = page.locator('[data-testid="calendar-link"], a:has-text("Calendar"), a:has-text("Appointment"), button:has-text("Schedule")').first();
