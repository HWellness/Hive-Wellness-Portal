...
test.describe('Client User Journey', () => {
  test('should login as client and access portal', async ({ page }) => {
    ...
    await expect(page.locator('text=/demo|client|dashboard/i').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should browse available therapists', async ({ page }) => {
    ...
    const therapistLink = page.locator('[data-testid="browse-therapists"], a:has-text("Therapist"), a:has-text("Find"), button:has-text("Browse")').first();
    
    if (await therapistLink.isVisible({ timeout: 5000 })) {
      await therapistLink.click();
      
      // Wait for therapist list to load
      await expect(page.locator('[data-testid="therapist-card"], .therapist-card').first()).toBeVisible({ timeout: 10000 });
