...
async function fixMessagingService() {
  try {
    ...
    if (messagingServices.length === 0) {
      ...
      const phoneNumberResource = await client.messaging.services(newService.sid)
        .phoneNumbers
        .create({ phoneNumberSid: await getPhoneNumberSid(client, phoneNumber) });
      ...
      for (const service of messagingServices) {
        ...
        const phoneNumbers = await client.messaging.services(service.sid)
          .phoneNumbers.list();
        ...
        if (phoneNumbers.length === 0) {
          ...
          try {
            ...
            await client.messaging.services(service.sid)
              .phoneNumbers
          ...
          const ourNumberInService = phoneNumbers.find(num => 
...
async function getPhoneNumberSid(client, phoneNumber) {
  ...
  const found = phoneNumbers.find(num => 
    num.phoneNumber === phoneNumber || 
    num.phoneNumber.replace(/\s/g, '') === phoneNumber.replace(/\s/g, '')
  );
